...
public class RequestPermissionsScreenWithNotification extends BaseScreen implements UserActionRequired
{
  ...
  @NonNull private final Runnable mPermissionsGrantedCallback;

  public RequestPermissionsScreenWithNotification(
    @NonNull CarContext carContext, @NonNull Runnable permissionsGrantedCallback)
  {
    super(carContext);
  ...
  @RequiresPermission(value = Manifest.permission.POST_NOTIFICATIONS) private void sendPermissionsRequestNotification()
  {
    final int FLAG_IMMUTABLE = Build.VERSION.SDK_INT < Build.VERSION_CODES.M ? 0 : PendingIntent.FLAG_IMMUTABLE;
    final Intent contentIntent = new Intent(getCarContext(), RequestPermissionsActivity.class);
    final PendingIntent pendingIntent =
      PendingIntent.getActivity(getCarContext(), 0, contentIntent, PendingIntent.FLAG_CANCEL_CURRENT | FLAG_IMMUTABLE);
