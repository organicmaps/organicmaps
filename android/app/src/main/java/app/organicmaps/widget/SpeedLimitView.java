...
public class SpeedLimitView extends View
{
  ...
  private void drawCurrentSpeedText(@NonNull Canvas canvas, float cx, float cy, boolean alert) {
    ...
    float totalTextHeight = speedTextBounds.height() + unitsTextBounds.height();
    // Position speed text
    float speedTextY = cy - totalTextHeight / 2 + speedTextBounds.height();
    // Position units text
  ...
  private void configureCurrentSpeedTextSize() {
    ...
    float speedTextHeightRatio = 0.7f;
    ...
    float speedTextSize = findMaxTextSizeForHeight("188", mCurrentSpeedTextPaint, speedTextHeight);
