...
class ArrowPopupBackground(
    ...
    override fun draw(canvas: Canvas) {
        ...
        val arrowBase = if (arrowOnTop) b.top + arrowHeight else b.bottom - arrowHeight
        ...
        // Arrow shadow.
        path.reset()
        path.moveTo(cx - arrowWidth / 2 - shadowDy, arrowBase)
        path.lineTo(cx, shadowTip)
        path.lineTo(cx + arrowWidth / 2 + shadowDy, arrowBase)
        path.close()
        canvas.drawPath(path, shadowPaint)

        // Arrow triangle.
        path.reset()
        path.moveTo(cx - arrowWidth / 2, arrowBase)
        path.lineTo(cx, arrowTip)
        path.lineTo(cx + arrowWidth / 2, arrowBase)
        path.close()
        canvas.drawPath(path, paint)
    }

    override fun getPadding(padding: Rect): Boolean {
        val h = arrowHeight.toInt()
        if (arrowOnTop) {
            padding.set(0, h, 0, 0)
        } else {
            padding.set(0, 0, 0, h)
