...
public class PlacePageLinksFragment extends Fragment implements Observer<MapObject>
{
  ...
  private String getLink(@NonNull Metadata.MetadataType type)
  {
    ...
    case FMD_WEBSITE:
      return mMapObject.getWebsiteUrl();
  ...
  public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState)
  {
    ...
    mKayak.setOnClickListener((v) -> {
      final String url = Objects.requireNonNull(mMapObject.getKayakUrl());
    ...
    mEmail.setOnClickListener((v) -> Utils.sendTo(requireContext(),
        Objects.requireNonNull(mMapObject.getMetadata(Metadata.MetadataType.FMD_EMAIL))));
  ...
  private boolean copyUrl(View view, Metadata.MetadataType type)
  {
    ...
    final String title = type == Metadata.MetadataType.FMD_WEBSITE ?
        mMapObject.getWebsiteUrl() : mMapObject.getMetadata(type);
  ...
  private void refreshLinks()
  {
    ...
    final SharedPreferences sharedPrefs = MwmApplication.prefs(context);

    refreshMetadataOrHide(mMapObject.getWebsiteUrl(), mWebsite, mTvWebsite);
    ...
    final String facebook = sharedPrefs.getBoolean(getString(R.string.pref_show_facebook), true)
        ? mMapObject.getMetadata(Metadata.MetadataType.FMD_CONTACT_FACEBOOK) : null;
    refreshMetadataOrHide(facebook, mFacebookPage, mTvFacebookPage);

    final String instagram = sharedPrefs.getBoolean(getString(R.string.pref_show_instagram), true)
        ? mMapObject.getMetadata(Metadata.MetadataType.FMD_CONTACT_INSTAGRAM) : null;
    refreshMetadataOrHide(instagram, mInstagramPage, mTvInstagramPage);

    final String twitter = sharedPrefs.getBoolean(getString(R.string.pref_show_twitter), true)
        ? mMapObject.getMetadata(Metadata.MetadataType.FMD_CONTACT_TWITTER) : null;
    refreshMetadataOrHide(twitter, mTwitterPage, mTvTwitterPage);

    final String vk = sharedPrefs.getBoolean(getString(R.string.pref_show_vk), true)
        ? mMapObject.getMetadata(Metadata.MetadataType.FMD_CONTACT_VK) : null;
    refreshMetadataOrHide(vk, mVkPage, mTvVkPage);

    final String line = sharedPrefs.getBoolean(getString(R.string.pref_show_line), true)
        ? mMapObject.getMetadata(Metadata.MetadataType.FMD_CONTACT_LINE) : null;
    refreshMetadataOrHide(line, mLinePage, mTvLinePage);

    final String kayak = sharedPrefs.getBoolean(getString(R.string.pref_show_kayak), true)
        ? mMapObject.getKayakUrl() : null;
