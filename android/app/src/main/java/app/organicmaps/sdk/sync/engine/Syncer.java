...
public class Syncer
{
  ...
  public void performSync(File tempDir) throws SyncOpException, LockAlreadyHeldException
  {
    ...
    {
      if (cloudHasChanges)
        bidirectionalSync(editSession, tempDir);
      else
   ...
   */
  private void bidirectionalSync(SyncClient.EditSession editSession, File tempDir) throws SyncOpException
  {
    ...
    while (!mChangedFiles.isEmpty())
    {
      ...
      syncChangedFile(editSession, locallyChangedFile, cloudFilesState);
   ...
   */
  private void syncChangedFile(SyncClient.EditSession editSession, String locallyChangedFile,
