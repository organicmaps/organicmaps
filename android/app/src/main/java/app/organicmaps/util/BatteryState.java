...
public final class BatteryState
{
  ...
  @NonNull public static State getState(@NonNull Context context)
  {
  ...
  // Called from JNI.
  @Keep @SuppressWarnings("unused") @IntRange(from = 0, to = 100) public static int getLevel(@NonNull Context context)
  {
    return getState(context).getLevel();
  }

  @IntRange(from = 0, to = 100) private static int getLevel(@NonNull Intent batteryStatus)
  {
    return batteryStatus.getIntExtra(BatteryManager.EXTRA_LEVEL, 0);
  }

  // Called from JNI.
  @Keep @SuppressWarnings("unused") @ChargingStatus public static int getChargingStatus(@NonNull Context context)
  {
    return getState(context).getChargingStatus();
  }

  @ChargingStatus private static int getChargingStatus(@NonNull Intent batteryStatus)
  {
  ...
  {
    @IntRange(from = 0, to = 100) private final int mLevel;

    @ChargingStatus private final int mChargingStatus;
    ...
    @IntRange(from = 0, to = 100) public int getLevel()
    {
      return mLevel;
    }

    @ChargingStatus public int getChargingStatus()
    {
