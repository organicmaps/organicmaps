...
void InjectMetadata(JNIEnv * env, jclass const clazz, jobject const mapObject, osm::MapObject const & src)
{
  ...
  src.ForEachMetadataReadable(
    [env, &mapObject](osm::MapObject::MetadataID id, std::string const & meta)
    {
                       ...
                       jni::TScopedLocalRef const & routingPointInfo, jobject const & popularity)
{
  ...
  jni::TScopedLocalRef jWikiDescription(env, jni::ToJavaString(env, info.GetWikiDescription()));
  jobject mapObject =
    env->NewObject(g_bookmarkClazz, ctorId, jFeatureId.get(), static_cast<jlong>(categoryId),
                   static_cast<jlong>(bookmarkId), jTitle.get(), jSecondaryTitle.get(), jSubtitle.get(), jAddress.get(),
...
{
  static jclass const pointClass = jni::GetGlobalClassRef(env, "app/organicmaps/bookmarks/data/ElevationInfo$Point");
