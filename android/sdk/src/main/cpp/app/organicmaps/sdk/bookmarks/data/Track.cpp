...
jobjectArray BuildTrackCandidatesArray(JNIEnv * env, place_page::Info const & info)
{
  ...
  jclass candidateClazz = env->FindClass("app/organicmaps/sdk/bookmarks/data/TrackSelectionCandidate");
  static jmethodID const candidateCtor = jni::GetConstructorID(env, candidateClazz, "(JLjava/lang/String;IZ)V");

  jobjectArray result = env->NewObjectArray(static_cast<jsize>(candidates.size()), candidateClazz, nullptr);
  ...
  for (size_t i = 0; i < candidates.size(); ++i)
  {
    ...
    jni::TScopedLocalRef candidate(
        env, env->NewObject(candidateClazz, candidateCtor, static_cast<jlong>(c.m_trackId), title.get(),
  ...
  env->DeleteLocalRef(candidateClazz);
  return result;
}
}  // namespace
