                    ...
                    jni::TScopedLocalRef const & routingPointInfo)
{
  ...
  jobject mapObject = env->NewObject(g_trackClazz, ctorId,
    ...
    jni::ToJavaString(env, info.GetTitle()),
    jni::ToJavaString(env, info.GetSecondaryTitle()),
    jni::ToJavaString(env, info.GetSubtitle()),
    jni::ToJavaString(env, info.GetSecondarySubtitle()),
