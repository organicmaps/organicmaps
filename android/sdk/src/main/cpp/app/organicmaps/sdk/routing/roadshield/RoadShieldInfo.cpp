...
jobject ToJavaRoadShieldInfo(JNIEnv * env, routing::FollowingInfo::RoadShieldInfo const & roadShieldInfo)
{
  ...
  jobject const result = env->NewObject(klass, ctorRouteInfoID,
    roadShieldInfo.m_targetRoadShields.empty() ?
      nullptr :
      ToJavaRoadShieldsArray(env, roadShieldInfo.m_targetRoadShields),
    roadShieldInfo.m_junctionShields.empty() ?
      nullptr :
      ToJavaRoadShieldsArray(env, roadShieldInfo.m_junctionShields)
  );
  // clang-format on
