                                  ...
                                  bool launchByDeepLink, uint32_t appVersionCode, bool isCustomROM)
{
  ...
  // Drape only needs to know whether the crosshair is shown; SetChoosePositionMode() carries the per-mode rules.
                                                                                      ...
                                                                                      jobject listener)
{
  ...
  frm()->GetRoutingManager().SetRoutePointCallback(
      std::bind(&CallRoutePointCallbackListener, jni::make_global_ref(listener), std::placeholders::_1));
}
