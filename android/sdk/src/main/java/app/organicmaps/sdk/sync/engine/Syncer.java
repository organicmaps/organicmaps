...
public class Syncer
{
      ...
      throws SyncOpException
  {
    ...
    for (String filePath : cachedChecksums.keySet())
    {
      ...
      {
        onCategoryDeletedRemotely(filePath);
    ...
    File bmDir = BookmarkManager.getBookmarksDir();
    for (Map.Entry<String, String> cloudFileEntry : cloudFilesState.omBookmarkFiles().entrySet())
    {
      ...
      if (!cloudFileChecksum.equals(cachedChecksums.getOrDefault(localFilePath, null)))
      {
            ...
            && !Objects.equals(mSyncClient.computeLocalFileChecksum(localFilePath), cachedChecksums.get(localFilePath)))
        {
          ...
          BookmarkManager.addSuffixToCategory(localFilePath);
          asyncChanges = true;
        ...
        BookmarkManager.reloadBookmark(localFilePath);
      }
    }
                                  ...
                                  CloudFilesState cloudFilesState) throws SyncOpException
  {
    ...
    case Both ->
    {
      ...
      else
      {
        ...
        else
        {
          ...
          BookmarkManager.addSuffixToCategory(locallyChangedFile);
          asyncChanges = true;
    ...
    case OnlyLocal ->
    {
      ...
      if (Objects.equals(currentChecksum, lastSyncedChecksum))
      {
        ...
        if (!onCategoryDeletedRemotely(locallyChangedFile))
  ...
  private boolean onCategoryDeletedRemotely(String localFilePath)
  {
    if (FileUtils.deleteFile(localFilePath))
    {
      UiThread.run(() -> BookmarkManager.INSTANCE.deleteCategoryByFilename(localFilePath));
      SyncManager.INSTANCE.notifyOtherSyncers(mAccountId, localFilePath);
      return true;
    }
    else
