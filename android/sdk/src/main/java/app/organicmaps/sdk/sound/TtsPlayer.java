...
public enum TtsPlayer
{
  ...
  private static final int TTS_BLE_SCO_DELAY_MILLIS = 1500;
  ...
  private boolean speakFirstString(@NonNull String text)
  {
    ...
    final boolean isBluetoothScoUsed = mAudioFocusManager.isBluetoothScoDeviceUsed();
    final boolean isMusicActive = mAudioFocusManager.isMusicActive();

    boolean result = true;
    if (isBluetoothScoUsed)
      result = mTts.playSilentUtterance(TTS_BLE_SCO_DELAY_MILLIS, TextToSpeech.QUEUE_FLUSH, TTS_SILENT_UTTERANCE_ID)
            == TextToSpeech.SUCCESS;
    if (result)
      mTtsQueueSize.incrementAndGet();
    else
    {
      Logger.d(TAG, "Failed to play silent utterance for Bluetooth SCO delay");
      return false;
    }

    if (isMusicActive && !isBluetoothScoUsed)
    ...
    result = mTts.speak(text, isBluetoothScoUsed || isMusicActive ? TextToSpeech.QUEUE_ADD : TextToSpeech.QUEUE_FLUSH,
                        mParams, text)
          == TextToSpeech.SUCCESS;
    if (result)
      mTtsQueueSize.incrementAndGet();
