...
public class RoutingController
{
  ...
  private void armPoiPick(@NonNull RouteMarkType pointType, @Nullable Integer replaceStopIndex)
  {
    ...
    mCanPickMyPosition = computeCanPickMyPosition(pointType, replaceStopIndex);
  }
  ...
  private static boolean isReplacedByPick(@NonNull RouteMarkData point, @NonNull RouteMarkType pickType,
                                          @Nullable Integer replaceStopIndex)
  {
    if (point.mPointType != pickType)
      return false;
    if (pickType != RouteMarkType.Intermediate)
      return true;
    return replaceStopIndex != null && point.mIntermediateIndex == replaceStopIndex;
  }
  ...
  private static boolean computeCanPickMyPosition(@NonNull RouteMarkType pointType, @Nullable Integer replaceStopIndex)
  {
    for (RouteMarkData point : Framework.nativeGetRoutePoints())
      if (point.mIsMyPosition && !isReplacedByPick(point, pointType, replaceStopIndex))
        return false;
    return true;
  }
