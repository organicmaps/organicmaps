...
bool LatLon::operator<(ms::LatLon const & rhs) const { return std::tie(m_lat, m_lon) < std::tie(rhs.m_lat, rhs.m_lon); }
...
{
  return base::AlmostEqualAbs(ll1.m_lat, ll2.m_lat, eps) && base::AlmostEqualAbs(ll1.m_lon, ll2.m_lon, eps);
}
}  // namespace base
