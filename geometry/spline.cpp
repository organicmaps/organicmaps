...
size_t Spline::GetSize() const { return m_position.size(); }
...
bool Spline::IsEmpty() const { return m_position.empty(); }

bool Spline::IsValid() const { return m_position.size() > 1; }
...
double Spline::GetLength() const { return std::accumulate(m_length.begin(), m_length.end(), 0.0); }
...
bool Spline::iterator::IsAttached() const { return m_spl != nullptr; }
...
double Spline::iterator::GetFullLength() const { return m_spl->GetLength(); }

bool Spline::iterator::BeginAgain() const { return m_checker; }

double Spline::iterator::GetDistance() const { return m_dist; }

size_t Spline::iterator::GetIndex() const { return m_index; }
...
bool SharedSpline::IsNull() const { return m_spline == nullptr; }

void SharedSpline::Reset(Spline * spline) { m_spline.reset(spline); }
...
Spline const * SharedSpline::operator->() const { return Get(); }
