...
usage() {
  echo "This tool configures omaps with private repository or as an opensource build"
  echo "Usage:"
...
' > "$PRIVATE_NETWORK_CONFIG"
  echo '{"client": [{"api_key": [{"current_key": ""}], "client_info": {"mobilesdk_app_id": "omaps", "android_client_info": {"package_name": "app.organicmaps.debug"}}}], "configuration_version": "1", "project_info": {"storage_bucket": "", "project_id": "", "project_number": ""}}' > "$PRIVATE_GOOGLE_SERVICES"
}

setup_private() {
  echo "Setting up private configuration: repo '$PRIVATE_REPO', branch '$PRIVATE_BRANCH'"
  rm -rf "$TMP_REPO_DIR"
  git clone --depth 1 --no-single-branch "$PRIVATE_REPO" "$TMP_REPO_DIR"
