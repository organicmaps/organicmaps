...
usage() {
  ...
  echo ""
  echo "./configure.sh [debug|beta|release]"
}

TYPE=${1:-debug}
if [ ! -f "private/$TYPE/private.h" ]; then
   echo "Can find private/$TYPE configuration" >&2
  exit 1
fi
echo "Initializing repository with default values for $TYPE."
cp -pR ./private/$TYPE/* .
