...
UNIT_TEST(Facebook_GetBanner)
{
  ...
  ads::FacebookPoi facebook;
  {
    ...
    TEST_EQUAL(facebook.GetBanner(holder, "Brazil", "ru"), facebook.GetBannerForOtherTypes(), ());
    holder.Add(c.GetTypeByPath({"amenity", "pub"}));
    TEST_EQUAL(facebook.GetBanner(holder, "Cuba", "ru"), facebook.GetBannerForOtherTypes(), ());
  }
  {
    feature::TypesHolder holder;
    holder.Add(c.GetTypeByPath({"amenity", "restaurant"}));
    TEST_EQUAL(facebook.GetBanner(holder, "Any country", "ru"), facebook.GetBannerForOtherTypes(), ());
  }
  {
    feature::TypesHolder holder;
    holder.Assign(c.GetTypeByPath({"tourism", "information", "map"}));
    TEST_EQUAL(facebook.GetBanner(holder, "Russia", "ru"), facebook.GetBannerForOtherTypes(), ());
  }
  {
    feature::TypesHolder holder;
    holder.Assign(c.GetTypeByPath({"shop", "ticket"}));
    TEST_EQUAL(facebook.GetBanner(holder, "USA", "ru"), facebook.GetBannerForOtherTypes(), ());
  }
  {
    feature::TypesHolder holder;
    holder.Assign(c.GetTypeByPath({"amenity", "toilets"}));
    auto const bannerId = facebook.GetBannerForOtherTypes();
