...
void CompileShaders(CompilerData const & compiler, std::string const & additionalDefines = {})
{
  ...
  TestShaders(
    compiler.m_apiVersion, defines, ".vert", GetVertexShaders(compiler.m_apiVersion), compilerPath,
    [](QProcess const &) {}, argsPrepareFn, successChecker, ss);
  TestShaders(
    compiler.m_apiVersion, defines, ".frag", GetFragmentShaders(compiler.m_apiVersion), compilerPath,
    [](QProcess const &) {}, argsPrepareFn, successChecker, ss);
...
UNIT_TEST(MobileCompileShaders_Test)
{
  ...
  workerThread.Push([] { CompileShaders({dp::ApiVersion::OpenGLES2, GetCompilerPath(kCompilerOpenGLES)}); });

  workerThread.Push([] { CompileShaders({dp::ApiVersion::OpenGLES3, GetCompilerPath(kCompilerOpenGLES)}); });

  workerThread.Push(
    [] { CompileShaders({dp::ApiVersion::OpenGLES2, GetCompilerPath(kCompilerOpenGLES)}, "#define ENABLE_VTF\n"); });
  ...
  workerThread.Push(
    []
    {
      CompileShaders({dp::ApiVersion::OpenGLES2, GetCompilerPath(kCompilerOpenGLES)}, "#define SAMSUNG_GOOGLE_NEXUS\n");
    });

  workerThread.Push(
    []
    {
      CompileShaders({dp::ApiVersion::OpenGLES3, GetCompilerPath(kCompilerOpenGLES)}, "#define SAMSUNG_GOOGLE_NEXUS\n");
    });
                        ...
                        MaliReleaseVersion const & version)
{
  ...
  TestShaders(compiler.m_apiVersion, defines, {}, GetVertexShaders(compiler.m_apiVersion), compilerPath, procPrepare,
              argForming, successChecker, ss);
  shaderType = "-f";
  TestShaders(compiler.m_apiVersion, defines, {}, GetFragmentShaders(compiler.m_apiVersion), compilerPath, procPrepare,
              argForming, successChecker, ss);
...
#if defined(OMIM_OS_MAC)
    {dp::ApiVersion::OpenGLES2, GetCompilerPath(kCompilerMaliOpenGLES2), GetCompilerPath(kMaliCompilerOpenGLES2Dir),
     driversES2old},
#endif
    {dp::ApiVersion::OpenGLES2, GetCompilerPath(kCompilerMaliOpenGLES3), GetCompilerPath(kMaliCompilerOpenGLES3Dir),
     driversES2new},
  ...
  for (auto const & compiler : compilers)
  {
    for (auto const & set : compiler.m_driverSets)
    {
      for (auto const & version : set.m_releases)
      {
        ...
        counter++;
        workerThread.Push(
          [&progressCounter, compiler, set, version]
          {
