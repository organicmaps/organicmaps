                                                   ...
                                                   Direction direction, std::vector<m2::PointD> & polyline)
{
  ...
  {
    proj =
      std::min_element(projections.begin(), projections.end(), [](ProjectionData const & p1, ProjectionData const & p2)
                       { return p1.m_distFromPoint < p2.m_distFromPoint; });
  }
...
bool StopIndexIsSet(size_t stopIndex) { return stopIndex != std::numeric_limits<size_t>::max(); }
