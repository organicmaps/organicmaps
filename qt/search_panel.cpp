  ...
  else if (m_mode == Mode::Viewport)
  {
    ViewportSearchParams params{normalized,
                                ...
                                [this](search::Results results)
                                {
                                  ...
                                  // initiate an arbitrary amount of actual search requests with different viewports,
                                  // and clearing the table would require additional care (or, most likely, we would
                                  // need a better API). This is similar to the Android and iOS clients where we do not
                                  // show the list of results in the viewport search mode.
...
void SearchPanel::hideEvent(QHideEvent *) { GetFramework().GetSearchAPI().CancelSearch(search::Mode::Everywhere); }
...
void SearchPanel::OnClearButton() { m_pEditor->setText(""); }

Framework & SearchPanel::GetFramework() const { return m_pDrawWidget->GetFramework(); }
