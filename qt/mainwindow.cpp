...
void MainWindow::LocationStateModeChanged(location::EMyPositionMode mode)
{
  if (mode == location::PendingPosition)
  {
    ...
    m_pMyPositionAction->setIcon(qt::Icon::LocationSearch);
    m_pMyPositionAction->setToolTip(tr("Looking for position..."));
    return;
  }

  m_pMyPositionAction->setIcon(qt::Icon::Location);
...
void MainWindow::CreateNavigationBar()
{
...

  {
    ...
    // m_layers->addAction(qt::Icon::Traffic, tr("Traffic"),
    //                     std::bind(&MainWindow::OnLayerEnabled, this, TRAFFIC), true);
    // m_layers->setChecked(TRAFFIC, Framework::LoadTrafficEnabled());

    m_layers->addAction(qt::Icon::Subway, tr("Public transport"), std::bind(&MainWindow::OnLayerEnabled, this, TRANSIT),
                        true);
    m_layers->setChecked(TRANSIT, Framework::LoadTransitSchemeEnabled());

    m_layers->addAction(qt::Icon::Isolines, tr("Isolines"), std::bind(&MainWindow::OnLayerEnabled, this, ISOLINES),
                        true);
    ...
    m_layers->addAction(qt::Icon::Isolines, tr("Outdoors"), std::bind(&MainWindow::OnLayerEnabled, this, OUTDOORS),
                        true);
    m_layers->setChecked(OUTDOORS, Framework::LoadOutdoorsEnabled());

    m_layers->addAction(qt::Icon::Isolines, tr("Hiking"), std::bind(&MainWindow::OnLayerEnabled, this, HIKING), true);
    m_layers->setChecked(HIKING, Framework::IsHikingEnabled());

    m_layers->addAction(qt::Icon::Isolines, tr("Cycling"), std::bind(&MainWindow::OnLayerEnabled, this, CYCLING), true);
    m_layers->setChecked(CYCLING, Framework::IsCyclingEnabled());

    pToolBar->addWidget(m_layers->create());
    m_layers->setMainIcon(qt::Icon::Layers);

    pToolBar->addSeparator();

    pToolBar->addAction(qt::Icon::Bookmark, tr("Show bookmarks and tracks; use ALT + RMB to add a bookmark"), this,
    ...
    m_routing->addAction(qt::Icon::PointStart, tr("Start point"),
                         std::bind(&MainWindow::OnRoutePointSelected, this, RouteMarkType::Start), false);
    m_routing->addAction(qt::Icon::PointIntermediate, tr("Intermediate point"),
                         std::bind(&MainWindow::OnRoutePointSelected, this, RouteMarkType::Intermediate), false);
    m_routing->addAction(qt::Icon::PointFinish, tr("Finish point"),
    ...
    QAction * act = pToolBar->addAction(qt::Icon::Routing, tr("Follow route"), this, SLOT(OnFollowRoute()));
    act->setToolTip(tr("Build route and use ALT + LMB to emulate current position"));
    pToolBar->addAction(qt::Icon::ClearRoute, tr("Clear route"), this, SLOT(OnClearRoute()));
    pToolBar->addAction(qt::Icon::SettingsRouting, tr("Routing settings"), this, SLOT(OnRoutingSettings()));

    pToolBar->addSeparator();

    m_pCreateFeatureAction =
        pToolBar->addAction(qt::Icon::Select, tr("Create Feature"), this, SLOT(OnCreateFeatureClicked()));
    ...
    m_selection->addAction(qt::Icon::SelectMode, tr("Roads selection mode"),
                           std::bind(&MainWindow::OnSwitchSelectionMode, this, SelectionMode::Features), true);
    m_selection->addAction(qt::Icon::CityBoundaries, tr("City boundaries selection mode"),
                           std::bind(&MainWindow::OnSwitchSelectionMode, this, SelectionMode::CityBoundaries), true);
    m_selection->addAction(qt::Icon::CityRoads, tr("City roads selection mode"),
                           std::bind(&MainWindow::OnSwitchSelectionMode, this, SelectionMode::CityRoads), true);
    m_selection->addAction(qt::Icon::Test, tr("Cross MWM segments selection mode"),
                           std::bind(&MainWindow::OnSwitchSelectionMode, this, SelectionMode::CrossMwmSegments), true);
    m_selection->addAction(qt::Icon::BordersSelection, tr("MWMs borders selection mode"), this,
    ...
    pToolBar->addAction(qt::Icon::Clear, tr("Clear selection"), this, SLOT(OnClearSelection()));
    ...
    m_pSearchAction = pToolBar->addAction(qt::Icon::Search, tr("Offline Search"), this, SLOT(OnSearchButtonClicked()));
    m_pSearchAction->setCheckable(true);
    m_pSearchAction->setShortcut(QKeySequence::Find);

    m_rulerAction = pToolBar->addAction(qt::Icon::Ruler, tr("Ruler"), this, SLOT(OnRulerEnabled()));
    ...
    m_pMyPositionAction = pToolBar->addAction(qt::Icon::Location, tr("My Position"), this, SLOT(OnMyPosition()));
    ...
    if (!m_mapcssFilePath.isEmpty())
    {
      m_pBuildStyleAction = pToolBar->addAction(qt::Icon::Run, tr("Build style"), this, SLOT(OnBuildStyle()));
      m_pBuildStyleAction->setCheckable(false);
      m_pBuildStyleAction->setToolTip(tr("Build style"));

      m_pRecalculateGeomIndex =
          pToolBar->addAction(qt::Icon::Geom, tr("Recalculate geometry index"), this, SLOT(OnRecalculateGeomIndex()));
    ...
    m_pDrawDebugRectAction = pToolBar->addAction(qt::Icon::Bug, tr("Debug style"), this, SLOT(OnDebugStyle()));
    ...
    m_pGetStatisticsAction = pToolBar->addAction(qt::Icon::Chart, tr("Get statistics"), this, SLOT(OnGetStatistics()));
    m_pGetStatisticsAction->setCheckable(false);
    m_pGetStatisticsAction->setToolTip(tr("Get statistics"));

    // Add "Run tests" button
    m_pRunTestsAction = pToolBar->addAction(qt::Icon::Test, tr("Run tests"), this, SLOT(OnRunTests()));
    m_pRunTestsAction->setCheckable(false);
    m_pRunTestsAction->setToolTip(tr("Run tests"));

    // Add "Build phone package" button
    m_pBuildPhonePackAction =
        pToolBar->addAction(qt::Icon::PhonePack, tr("Build phone package"), this, SLOT(OnBuildPhonePackage()));
...
#ifndef NO_DOWNLOADER
  pToolBar->addSeparator();
  pToolBar->addAction(qt::Icon::Download, tr("Download Maps"), this, SLOT(ShowUpdateDialog()));
#endif  // NO_DOWNLOADER
