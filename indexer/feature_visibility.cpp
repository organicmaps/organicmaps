...
{
  keys.erase_if(
    [&f, zoomLevel](drule::Key const & key)
    {
...
bool IsCategoryNondrawableType(uint32_t type) { return TypeAlwaysExists(type); }

bool IsUsefulType(uint32_t type) { return IsUsefulNondrawableType(type) || classif().GetObject(type)->IsDrawableAny(); }
...
int GetMinDrawableScale(FeatureType & ft) { return GetMinDrawableScale(TypesHolder(ft), ft.GetLimitRectChecked()); }
