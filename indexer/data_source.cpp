...
{
public:
  template <class FnT> ReadMWMFunctor(FeatureSourceFactory const & factory, FnT && fn)
    : m_factory(factory), m_fn(std::forward<FnT>(fn))
  {
  }

  ReadMWMFunctor(FeatureSourceFactory const & factory, DataSource::FeatureCallback const & fn,
                 DataSource::StopSearchCallback stop = {})
    : m_factory(factory), m_stop(std::move(stop))
  {
    m_fn = [&fn](uint32_t index, FeatureSource & src) {
      ReadFeatureType(fn, src, index);
    };
  }
...
void DataSource::ForEachInRectForMWM(FeatureCallback const & f, m2::RectD const & rect, int scale, MwmId const & id) const
{
