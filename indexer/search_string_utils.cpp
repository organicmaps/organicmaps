...
public:
  ...
  bool MatchWithMisprints(DFA const & dfa) const
  {
    ...
    while (!q.empty())
    {
      ...
      currTrieIt.ForEachMove(
        [&q, &currDfaIt](UniChar const & c, TrieIt const & nextTrieIt)
        {
...
UniString GetStreetNameAsKey(std::string_view name, bool ignoreStreetSynonyms)
{
  ...
  Tokenize(name, kStreetTokensSeparator,
           [&](std::string_view v)
           {
...
bool IsStreetSynonymPrefix(UniString const & s) { return StreetsSynonymsHolder::Instance().MatchPrefix(s); }
