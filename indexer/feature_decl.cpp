...
namespace feature
{
std::string DebugPrint(GeomType type) { return ToString(type); }
...
std::string FeatureID::GetMwmName() const { return IsValid() ? m_mwmId.GetInfo()->GetCountryName() : std::string(); }

int64_t FeatureID::GetMwmVersion() const { return IsValid() ? m_mwmId.GetInfo()->GetVersion() : -1; }
...
bool FeatureID::IsWorld() const { return m_mwmId.GetInfo()->GetType() == MwmInfo::MwmTypeT::WORLD; }
