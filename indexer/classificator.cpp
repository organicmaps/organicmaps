...
uint8_t GetValue(uint32_t type, uint8_t level) { return get_value(type, level); }
...
uint8_t GetLevel(uint32_t type) { return get_control_level(type); }
}  // namespace ftype
...
bool ClassifObject::IsDrawable(int scale) const { return (m_visibility[scale] && IsDrawableAny()); }

bool ClassifObject::IsDrawableAny() const { return (m_visibility != VisibleMask() && !m_drawRules.empty()); }
...
void Classificator::ReadTypesMapping(std::istream & s) { m_mapping.Load(s); }
