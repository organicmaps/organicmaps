...
LineRuleProto const * BaseRule::GetLine() const { return 0; }

AreaRuleProto const * BaseRule::GetArea() const { return 0; }

SymbolRuleProto const * BaseRule::GetSymbol() const { return 0; }

CaptionRuleProto const * BaseRule::GetCaption() const { return 0; }

PathTextRuleProto const * BaseRule::GetPathtext() const { return 0; }

ShieldRuleProto const * BaseRule::GetShield() const { return nullptr; }
...
void BaseRule::SetSelector(unique_ptr<ISelector> && selector) { m_selector = std::move(selector); }

RulesHolder::RulesHolder()
  : m_bgColors(scales::UPPER_STYLE_SCALE + 1, DEFAULT_BG_COLOR)
{}

RulesHolder::~RulesHolder() { Clean(); }
...
RulesHolder & rules() { return rules(GetStyleReader().GetCurrentStyle()); }
...
public:
  explicit Area(AreaRuleProto const & r)
    : m_area(r)
  {}
...
public:
  explicit Symbol(SymbolRuleProto const & r)
    : m_symbol(r)
  {}
...
public:
  explicit PathText(PathTextRuleProto const & r)
    : m_pathtext(r)
  {}
...
public:
  explicit Shield(ShieldRuleProto const & r)
    : m_shield(r)
  {}
...
private:
  ...
  public:
    ...
    RandI()
      ...
      , m_index(-1)
    {}
    RandI(ContainerProto const & cont, int ind)
      : m_cont(&cont)
      , m_index(ind)
    {}
