...
// ResultsEdits::Editor ----------------------------------------------------------------------------
ResultsEdits::Editor::Editor(ResultsEdits & parent, size_t index)
  : m_parent(parent)
  , m_index(index)
{}

bool ResultsEdits::Editor::Set(Relevance relevance) { return m_parent.SetRelevance(m_index, relevance); }
...
ResultsEdits::Entry::Type ResultsEdits::Editor::GetType() const { return m_parent.Get(m_index).m_type; }

// ResultsEdits ------------------------------------------------------------------------------------
void ResultsEdits::Apply()
{
  WithObserver(Update::MakeAll(),
               [this]()
               {
...
{
  return WithObserver(Update::MakeSingle(index),
                      [this, index, relevance]()
                      {
...
{
  return WithObserver(Update::MakeDelete(index),
                      [this, index]()
                      {
                        CHECK_LESS(index, m_entries.size(), ());
...
{
  return WithObserver(Update::MakeResurrect(index),
                      [this, index]()
                      {
                        CHECK_LESS(index, m_entries.size(), ());
