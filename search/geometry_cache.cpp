...
{
  auto p =
    FindOrCreateEntry(context.GetId(),
                      [&rect, &scale](Entry const & entry)
                      {
                        return scale == entry.m_scale && (entry.m_rect.IsRectInside(rect) ||
...
{
  auto p =
    FindOrCreateEntry(context.GetId(), [&rect, &scale](Entry const & entry)
