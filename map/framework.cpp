...
void Framework::ShowTrack(kml::TrackId trackId)
{
  ...
  if (m_drapeEngine)
    m_drapeEngine->SetModelViewCenter(rect.Center(), scales::GetScaleLevel(rect), true /* isAnim */,
...
ElevationInfo const & Framework::GetTrackRecordingElevationInfo() { return GpsTracker::Instance().GetElevationInfo(); }
...
bool Framework::IsTrackRecordingEmpty() const { return GpsTracker::Instance().IsEmpty(); }

bool Framework::IsTrackRecordingEnabled() const { return GpsTracker::Instance().IsEnabled(); }

void Framework::SaveRoute() { m_routingManager.SaveRoute(); }
...
std::string Framework::GetParsedOAuth2Code() const { return m_parsedMapApi.GetOAuth2Code(); }
...
std::string Framework::GetMapLanguageCode() { return languages::GetCurrentMapLanguage(); }
