...
/**
 The Inbox message type. Plain = without any action, Richmedia = contains a Rich media page, URL =
 contains remote URL, Deeplink = contains Deeplink
 */
...
//! Action of the Inbox Message is performed (if true, an action was performed in the Inbox see +
//! (void)performActionForMessageWithCode:(NSString *)code or an action was performed on the push
//! tap )
...
/**
 ...
 @param completion - if successful, return the number of the InboxMessages with no action performed.
 Otherwise, return error
 */
+ (void)messagesWithNoActionPerformedCountWithCompletion:(void (^)(NSInteger count,
                                                                   NSError * error))completion;

/**
 Get the number of the unread PWInboxMessageProtocol

 @param completion - if successful, return the number of the unread InboxMessages. Otherwise, return
 error
 */
...
/**
 ...
 @param completion - if successful, return the total number of the InboxMessages. Otherwise, return
 error
 */
...
/**
 ...
 @param completion - if successful, return the collection of the InboxMessages. Otherwise, return
 error
 */
+ (void)loadMessagesWithCompletion:(void (^)(NSArray<NSObject<PWInboxMessageProtocol> *> * messages,
                                             NSError * error))completion;
...
/**
 Call this method, when the user clicks on the InboxMessageProtocol and the message’s action is
 performed
...
/**
 Subscribe for messages arriving with push notifications. @warning You need to unsubscribe by
 calling the removeObserver method, if you don't want to receive notifications

 @param completion - return the collection of the InboxMessages.
 */
+ (id<NSObject>)addObserverForDidReceiveInPushNotificationCompletion:
    (void (^)(NSArray<NSObject<PWInboxMessageProtocol> *> * messagesAdded))completion;

/**
 Subscribe for messages arriving when a message is deleted, added, or updated. @warning You need to
 unsubscribe by calling the removeObserver method, if you don't want to receive notifications

 @param completion - return the collection of the InboxMessages.
 */
+ (id<NSObject>)addObserverForUpdateInboxMessagesCompletion:
    (void (^)(NSArray<NSString *> * messagesDeleted,
