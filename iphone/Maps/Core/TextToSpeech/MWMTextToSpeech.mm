...
// Voices reading out `language`, one per system-provided name. Some voices, notably the Eloquence set
// (Eddy, Flo, Grandma, ...), have the same name in every region of the language. Keep the variant
// closest to the device language so that the accent matches what the user hears elsewhere in the
// system.
...
{
  [[self tts] invalidateVoiceCatalog];
}

- (void)invalidateVoiceCatalog
{
  ASSERT(NSThread.isMainThread, ());
  // Rebuilding the catalog costs tens of milliseconds, and most updates do not need to read it.
  self.catalog = nil;
...
- (void)availableVoicesDidChange:(NSNotification *)notification
{
  if (NSThread.isMainThread)
  {
    [self invalidateVoiceCatalog];
    return;
  }
  dispatch_async(dispatch_get_main_queue(), ^{ [self invalidateVoiceCatalog]; });
}
...
// Initialization and catalog refresh preserve a pinned voice. Explicit language selection clears
// the pin in +setLanguage:, while -createVoice: ignores a pin that cannot read this locale.
...
{
  // Recreate the voice after catalog invalidation so callers such as the settings screen never
  // observe a transient nil while the installed voices are being re-enumerated.
  if (!self.speechVoice)
...
{
  MWMTextToSpeech * tts = [MWMTextToSpeech tts];
  NSArray<MWMTTSVoice *> * listed = [tts.catalog listedVoicesReading:language.code];
  AVSpeechSynthesisVoice * current = [tts speechVoiceInUse];
  if (!current || TwineLanguage(current.language) != language.code.UTF8String)
    return listed;

  // A saved accent can differ from the device's preferred variant. Keep the voice actually in use
  // visible and previewable without adding another row for the same name.
  for (NSUInteger i = 0; i < listed.count; ++i)
    if ([listed[i].title isEqualToString:current.name] && ![listed[i].identifier isEqualToString:current.identifier])
    {
      NSMutableArray<MWMTTSVoice *> * result = [listed mutableCopy];
      result[i] = VoiceFromSpeechVoice(current);
      return result;
    }
  return listed;
}
