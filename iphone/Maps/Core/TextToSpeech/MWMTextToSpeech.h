...
// Posted when the set of voices available to the app may have changed. Screens listing languages or
// voices should reload on it.
...
// The system-provided name shown in the UI, including its quality suffix when present.
@property(nonatomic, readonly) NSString * title;
...
// Accessed on the main thread, like the routing framework and the settings UI it coordinates.
...
// Persisted preferred BCP-47 tag, e.g. "en-GB". The current voice can use a fallback when the saved
// language is unavailable; use +currentLanguage and +currentVoice for the effective selection.
...
// The language the notifications are read in, nil when no supported fallback voice is available.
...
// selected language, otherwise the best available fallback. Nil when no supported fallback exists.
+ (MWMTTSVoice * _Nullable)currentVoice;
// Selects the voice and, with it, the language it reads out.
+ (void)setVoice:(MWMTTSVoice *)voice;
// Whether the voice in use was pinned by hand with +setVoice:, rather than picked as the best
// installed one for the language. The voice picker offers going back to the automatic choice.
+ (BOOL)isVoicePinned;
...
// runs on the main queue when the preview ends, including when navigation cuts it short, but not
// when -stopPreview ends it.
