@objc @objcMembers class DeepLinkHandler: NSObject {
  ...
  private func handleFileImport(url: URL, openInPlace: Bool) -> Bool {
    ...
    guard openInPlace else {
      DeepLinkParser.addBookmarksFile(url, isTemporaryFile: false)
      reset()
      return true
    }
  ...
  private func copyFileToTemporaryDirectory(_ url: URL) throws -> URL {
    let temporaryDirectory = FileManager.default.temporaryDirectory.appendingPathComponent("FileImports", isDirectory: true)
    try FileManager.default.createDirectory(at: temporaryDirectory, withIntermediateDirectories: true)

    let fileName = "\(url.deletingPathExtension().lastPathComponent)-\(UUID().uuidString)"
    var localCopyURL = temporaryDirectory.appendingPathComponent(fileName)
    if !url.pathExtension.isEmpty {
      localCopyURL.appendPathExtension(url.pathExtension)
    }
