...
void AuthorizationStoreCredentialsWithOAuth(std::string const & code)
{
  std::string const token = osm::OsmOAuth::ServerAuth().FinishAuthorization(code);
  AuthorizationStoreCredentials(token);
}
