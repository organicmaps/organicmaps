...
class PlacePageInfoViewController: UIViewController {
  ...
  private var selectedRelId: UInt32?
  ...
  private func setupViews() {
    ...
    if let routes = placePageInfoData.routes, !routes.isEmpty {
      let refsString = routes.map { $0.ref }.joined(separator: " • ")
      routeRefsView = createInfoItem(refsString,
  ...
  private func showRoutesPopup(for routes: [PlacePageRoute]) {
    ...
    for route in routes {
      ...
      alert.addAction(UIAlertAction(title: label, style: .default, handler: { [weak self] _ in
        ...
        self?.selectedRelId = route.relId
