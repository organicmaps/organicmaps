final class PlacePageExpandableDetailsSectionPresenter {
  ...
  init(viewModel: PlacePageExpandableDetailsSectionViewModel, isHTML: Bool = false) {
    self.viewModel = viewModel
    self.isHTML = isHTML
  }

  func process(_ response: PlacePageExpandableDetailsSectionResponse) {
    let newViewModel = resolve(response, with: viewModel)
  ...
  private func resolve(_ response: PlacePageExpandableDetailsSectionResponse, with previousViewModel: PlacePageExpandableDetailsSectionViewModel) -> PlacePageExpandableDetailsSectionViewModel {
    ...
    switch response {
    case .initialize:
      if isHTML, let expandableText = viewModel.expandableText {
        Self.buildAttributedString(from: expandableText) { [weak self] attributedString in
