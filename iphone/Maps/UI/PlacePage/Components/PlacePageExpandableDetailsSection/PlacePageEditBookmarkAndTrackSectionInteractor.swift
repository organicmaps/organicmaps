...
final class PlacePageEditBookmarkAndTrackSectionInteractor: PlacePageExpandableDetailsSectionInteractor {
  ...
  private func reloadData() {
    ...
    presenter.process(.updateTitle(category ?? ""))
    presenter.process(.updateIcon(editColorImage))
    presenter.process(.updateAccessory(.ic24PxEdit))
    presenter.process(.updateExpandableText(description, isHTML: isHtmlDescription))
  }
