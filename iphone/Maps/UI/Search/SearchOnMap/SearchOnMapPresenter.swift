final class SearchOnMapPresenter {
  ...
  func process(_ response: SearchOnMap.Response) {
    ...
    if case .updateRoutePointActions(let actions) = response, viewModel.presentationStep == .hidden {
      // Keep the actions current without changing the active map picker state.
