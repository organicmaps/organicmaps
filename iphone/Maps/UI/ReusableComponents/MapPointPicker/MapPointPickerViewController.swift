...
final class MapPointPickerViewController: UIViewController {
  ...
  func present(in parentViewController: UIViewController) {
    ...
    // Keep zoom and recenter controls available while positioning the crosshair.
    // Cross-platform alignment is tracked in https://github.com/organicmaps/organicmaps/issues/13602.
  ...
  private func dismiss(completion: (() -> Void)?) {
    guard !isDismissing else {
      // A parent close invalidates a button result captured before the dismissal animation finishes.
      if completion == nil {
