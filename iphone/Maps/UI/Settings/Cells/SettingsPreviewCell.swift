...
final class SettingsPreviewCell: MWMTableViewCell {
  ...
  func configure(delegate: SettingsPreviewCellDelegate,
    ...
    if isSelected, showsDisclosure {
      accessoryType = .none
      accessoryView = Self.checkmarkAndDisclosureView()
    } else {
      accessoryView = nil
  ...
  /// The system accessory shows either a checkmark or a disclosure indicator; a selected row that
  /// also opens a list needs both.
  private static func checkmarkAndDisclosureView() -> UIView {
    ...
    let disclosure = UIImageView(image: UIImage(systemName: "chevron.right", withConfiguration: configuration))
    disclosure.tintColor = .blackHintText
    let stack = UIStackView(arrangedSubviews: [checkmark, disclosure])
