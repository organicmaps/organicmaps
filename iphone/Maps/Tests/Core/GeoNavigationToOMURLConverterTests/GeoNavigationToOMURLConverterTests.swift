...
final class GeoNavigationToOMURLConverterTests: XCTestCase {
  ...
  func test_GivenDirections_WhenDestinationWithoutSource_ThenFail() {
    let geoUrl = geoURLStub("geo-navigation://directions?destination=\(coordinate2)")
    XCTAssertNil(converter.convert(geoUrl))
  }
  ...
  func test_GivenPlace_WhenHasCoordinatesWithoutAddress_ThenShowPlace() {
    let geoUrl = geoURLStub("geo-navigation://place?coordinate=\(coordinate1)")
    XCTAssertEqual(converter.convert(geoUrl)?.absoluteString, "om://map?v=1&ll=\(coordinate1)")
  }
