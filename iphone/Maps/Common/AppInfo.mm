...
- (NSString *)deviceModel
{
  if (!_deviceModel)
  {
    ...
    _deviceModel = kDeviceModelsBeforeMetalDriver[machine];
    if (!_deviceModel)
      _deviceModel = kDeviceModelsWithiOS10MetalDriver[machine];
    if (!_deviceModel)
      _deviceModel = kDeviceModelsWithMetalDriver[machine];
    else
...
- (MWMOpenGLDriver)openGLDriver
{
  ...
  uname(&systemInfo);
  NSString * machine = @(systemInfo.machine);
  if (kDeviceModelsBeforeMetalDriver[machine] != nil)
    return MWMOpenGLDriverRegular;
  if (kDeviceModelsWithiOS10MetalDriver[machine] != nil)
  {
