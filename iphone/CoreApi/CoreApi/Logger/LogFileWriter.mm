...
- (BOOL)writeData:(NSData *)data error:(NSError * __autoreleasing _Nullable * _Nullable)error
{
  ...
  if (self.currentFileSize > 0 &&
      (self.currentFileSize >= self.maxFileSize || dataSize > self.maxFileSize - self.currentFileSize))
  {
    if (![self rotateWithError:error])
      return NO;
  }
...
{
  NSMutableArray<NSString *> * copies = [NSMutableArray arrayWithCapacity:2];
  for (NSString * sourcePath in @[self.rotatedFilePath, self.currentFilePath])
  {
    BOOL const isCurrentFile = [sourcePath isEqualToString:self.currentFilePath];
    if (!isCurrentFile && ![self.fileManager fileExistsAtPath:sourcePath])
      continue;
...
- (BOOL)rotateWithError:(NSError * __autoreleasing _Nullable * _Nullable)error
{
  ...
  if (![self.fileManager createFileAtPath:self.currentFilePath contents:nil attributes:nil])
    return SetError(error, LogFileWriterErrorOpenFailed, @"Failed to create a new current log file.");
