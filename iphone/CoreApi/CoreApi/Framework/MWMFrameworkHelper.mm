...
{
  auto & f = GetFramework();
  auto const newTheme = ^MapStyleTheme(MWMTheme theme) {
    switch (theme)
    {
    case MWMThemeDay:
  ...
  if (newTheme != MapStyleManager::Instance().GetCurrentTheme())
    f.SetMapStyle(MapStyleManager::Instance().GetCurrentStyleName(), newTheme);
}
