...
void EditorTest::GetFeatureTypeInfoTest()
{
  ...
  auto const mwmId = ConstructTestMwm(
  ...
  ForEachCafeAtPoint(m_dataSource, m2::PointD(1.0, 1.0),
                     [&editor](FeatureType & ft)
                     {
                       ...
                       auto const fti =
                         editor.GetFeatureTypeInfo(*featuresAfter, ft.GetID().m_mwmId, ft.GetID().m_index);
...
void EditorTest::SetIndexTest()
{
  ...
  auto const gbMwmId = BuildMwm("GB",
                                [](TestMwmBuilder & builder)
                                {
...
void EditorTest::GetFeatureStatusTest()
{
  ...
  ForEachCafeAtPoint(m_dataSource, m2::PointD(2.0, 2.0),
                     [&editor](FeatureType & ft)
                     {
                       TEST_EQUAL(editor.GetFeatureStatus(ft.GetID()), FeatureStatus::Untouched, ());
...
void EditorTest::GetFeaturesByStatusTest()
{
  ...
  ForEachCafeAtPoint(m_dataSource, m2::PointD(1.0, 1.0),
                     [&editor, &modifiedId](FeatureType & ft)
                     {
                       osm::EditableMapObject emo;
                       FillEditableMapObject(editor, ft, emo);
                       SetBuildingLevels(emo, "1");
  ...
  ForEachCafeAtPoint(m_dataSource, m2::PointD(2.0, 2.0),
                     [&editor, &deletedId](FeatureType & ft)
                     {
                       editor.DeleteFeature(ft.GetID());
                       deletedId = ft.GetID();
                     });

  ForEachCafeAtPoint(m_dataSource, m2::PointD(3.0, 3.0),
                     [&editor, &obsoleteId](FeatureType & ft)
                     {
...
void EditorTest::OnMapDeregisteredTest()
{
  ...
  auto const rfMwmId = BuildMwm("RF",
                                [](TestMwmBuilder & builder)
                                {
...
void EditorTest::RollBackChangesTest()
{
  ...
  ForEachCafeAtPoint(m_dataSource, m2::PointD(1.0, 1.0),
                     [&editor, &houseNumber](FeatureType & ft)
                     {
                       osm::EditableMapObject emo;
                       FillEditableMapObject(editor, ft, emo);
...
void EditorTest::ForEachFeatureInMwmRectAndScaleTest()
{
  ...
  ForEachCafeAtPoint(m_dataSource, m2::PointD(5.0, 5.0),
                     [](FeatureType & ft)
                     {
                       auto & editor = osm::Editor::Instance();
                       TEST_EQUAL(editor.GetFeatureStatus(ft.GetID()), FeatureStatus::Untouched, ());
...
void EditorTest::CreateNoteTest()
{
  ...
  ForEachCafeAtPoint(m_dataSource, m2::PointD(1.0, 1.0),
                     [&editor, &createAndCheckNote](FeatureType & ft)
                     {
...
void EditorTest::LoadMapEditsTest()
{
  ...
  ForEachCafeAtPoint(m_dataSource, m2::PointD(0.0, 0.0),
  ...
  ForEachCafeAtPoint(m_dataSource, m2::PointD(1.0, 1.0),
                     [&editor, &features](FeatureType & ft)
                     {
                       editor.DeleteFeature(ft.GetID());
                       features.emplace_back(ft.GetID());
                     });
  ...
  ForEachCafeAtPoint(m_dataSource, m2::PointD(7.0, 7.0),
                     [&features](FeatureType & ft)
                     {
                       SetBuildingLevelsToOne(ft);
                       features.emplace_back(ft.GetID());
  ...
  ForEachCafeAtPoint(m_dataSource, m2::PointD(6.0, 6.0),
                     [&features](FeatureType & ft)
                     {
                       SetBuildingLevelsToOne(ft);
                       features.emplace_back(ft.GetID());
  ...
  auto const newRfMwmId = BuildMwm(
    "RF",
    [](TestMwmBuilder & builder)
    {
      builder.Add(TestCafe(m2::PointD(2.0, 2.0), "Moscow Cafe1", "en"));
  ...
  auto const newGbMwmId = BuildMwm(
    "GB",
    [](TestMwmBuilder & builder)
    {
      builder.Add(TestCafe(m2::PointD(0.0, 0.0), "London Cafe", "en"));
...

  {
    ForEachCafeAtPoint(m_dataSource, m2::PointD(1.0, 1.0),
                       [&editor](FeatureType & ft)
                       {
                         ...
                         feature::TypesHolder typesHolder;
                         std::string defaultName;
                         editor.CreateNote({1.0, 1.0}, ft.GetID(), typesHolder, defaultName,
                                           NoteType::PlaceDoesNotExist, "exploded");
                       });
...
UNIT_CLASS_TEST(EditorTest, GetFeatureTypeInfoTest) { EditorTest::GetFeatureTypeInfoTest(); }

UNIT_CLASS_TEST(EditorTest, SetIndexTest) { EditorTest::SetIndexTest(); }

UNIT_CLASS_TEST(EditorTest, GetFeatureStatusTest) { EditorTest::GetFeatureStatusTest(); }

UNIT_CLASS_TEST(EditorTest, IsFeatureUploadedTest) { EditorTest::IsFeatureUploadedTest(); }

UNIT_CLASS_TEST(EditorTest, DeleteFeatureTest) { EditorTest::DeleteFeatureTest(); }

UNIT_CLASS_TEST(EditorTest, ClearAllLocalEditsTest) { EditorTest::ClearAllLocalEditsTest(); }

UNIT_CLASS_TEST(EditorTest, GetEditedFeatureStreetTest) { EditorTest::GetEditedFeatureStreetTest(); }

UNIT_CLASS_TEST(EditorTest, GetEditedFeatureTest) { EditorTest::GetEditedFeatureTest(); }

UNIT_CLASS_TEST(EditorTest, GetFeaturesByStatusTest) { EditorTest::GetFeaturesByStatusTest(); }

UNIT_CLASS_TEST(EditorTest, OnMapDeregisteredTest) { EditorTest::OnMapDeregisteredTest(); }

UNIT_CLASS_TEST(EditorTest, RollBackChangesTest) { EditorTest::RollBackChangesTest(); }

UNIT_CLASS_TEST(EditorTest, HaveMapEditsOrNotesToUploadTest) { EditorTest::HaveMapEditsOrNotesToUploadTest(); }

UNIT_CLASS_TEST(EditorTest, HaveMapEditsToUploadTest) { EditorTest::HaveMapEditsToUploadTest(); }

UNIT_CLASS_TEST(EditorTest, GetStatsTest) { EditorTest::GetStatsTest(); }

UNIT_CLASS_TEST(EditorTest, IsCreatedFeatureTest) { EditorTest::IsCreatedFeatureTest(); }

UNIT_CLASS_TEST(EditorTest, ForEachFeatureInMwmRectAndScaleTest) { EditorTest::ForEachFeatureInMwmRectAndScaleTest(); }

UNIT_CLASS_TEST(EditorTest, CreateNoteTest) { EditorTest::CreateNoteTest(); }

UNIT_CLASS_TEST(EditorTest, LoadMapEditsTest) { EditorTest::LoadMapEditsTest(); }

UNIT_CLASS_TEST(EditorTest, SaveEditedFeatureTest) { EditorTest::SaveEditedFeatureTest(); }
