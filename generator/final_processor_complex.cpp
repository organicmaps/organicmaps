...
void ComplexFinalProcessor::SetFilter(std::shared_ptr<FilterInterface> const & filter) { m_filter = filter; }

void ComplexFinalProcessor::SetGetNameFunction(hierarchy::GetNameFn const & getName) { m_getName = getName; }
...
void ComplexFinalProcessor::UseBuildingPartsInfo(std::string const & filename) { m_buildingPartsFilename = filename; }
...
void ComplexFinalProcessor::Process()
{
  ...
  ForEachMwmTmp(
    m_mwmTmpPath,
    [&](auto const & name, auto const & path)
    {
      ...
      if (m_buildingToParts)
      {
        hierarchy::AddChildrenTo(trees,
                                 [&](auto const & compositeId)
                                 {
