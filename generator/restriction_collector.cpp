                                                ...
                                                std::vector<uint32_t> & featureIds) const
{
  ...
  if (featureIds[0] == featureIds[1])
  {
    ...
    bool const viaIsFirstNode = base::AlmostEqualAbs(coords, mercator::FromLatLon(road.GetPoint(0)), kEps);
    bool const viaIsLastNode = base::AlmostEqualAbs(coords, mercator::FromLatLon(road.GetPoint(n - 1)), kEps);
...
void FromString(std::string_view str, double & number) { CHECK(strings::to_double(str, number), ()); }
}  // namespace routing_builder
