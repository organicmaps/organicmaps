...
public:
  explicit AccumulateStatistic(MapInfo & info)
    : m_info(info)
  {}
  ...
  void operator()(FeatureType & f, uint32_t)
  {
    ...
    if (f.GetGeomType() == feature::GeomType::Line)
    {
      ...
      f.ForEachPoint(
        [&len, &firstPoint, &lastPoint](m2::PointD const & pt)
        {
          if (firstPoint)
            firstPoint = false;
          else
...
std::string GetKey(ClassifType t) { return classif().GetReadableObjectName(t.m_val); }

std::string GetKey(AreaType t) { return strings::to_string(kAreas[t.m_val]); }
