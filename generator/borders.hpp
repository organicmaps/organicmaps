...
public:
  ...
  public:
    explicit ContainsCompareFn(double eps)
      : m_eps(eps)
      , m_squareEps(eps * eps)
    {}
    bool EqualPoints(m2::PointD const & p1, m2::PointD const & p2) const
    {
      return base::AlmostEqualAbs(p1.x, p2.x, m_eps) && base::AlmostEqualAbs(p1.y, p2.y, m_eps);
    }
    bool EqualZeroSquarePrecision(double val) const { return base::AlmostEqualAbs(val, 0.0, m_squareEps); }
  };
...
public:
  ...
  void ForEachCountryInRect(m2::RectD const & rect, ToDo && toDo) const
  {
    ...
    m_regionsTree.ForEachInRect(rect,
                                [&](auto const & countryPolygons)
                                {
