...
bool HierarchyPlace::Contains(m2::PointD const & point) const { return boost::geometry::covered_by(point, m_polygon); }

HierarchyLinker::HierarchyLinker(Node::Ptrs && nodes)
  : m_nodes(std::move(nodes))
  , m_tree(MakeTree4d(m_nodes))
{}
...
HierarchyLinker::Node::Ptr HierarchyLinker::FindPlaceParent(HierarchyPlace const & place)
{
  ...
  m_tree.ForEachInRect({point, point},
                       [&](auto const & candidateNode)
                       {
                         // https://wiki.openstreetmap.org/wiki/Simple_3D_buildings
                         // An object with tag 'building:part' is a part of a relation with outline 'building' or
                         // is contained in a object with tag 'building'. This case is second. We suppose a building
                         // part is only inside a building.
...
void HierarchyLinesBuilder::SetGetMainTypeFunction(GetMainTypeFn const & getMainType) { m_getMainType = getMainType; }
...
void FlattenBuildingParts(HierarchyLinker::Node::Ptrs & trees)
{
  for (auto & tree : trees)
  {
    ...
    for (auto const & buildingAndParts : buildingPartsTrees)
    {
      ...
      tree_node::PostOrderVisit(buildingAndParts.second,
                                [&](auto const & buildingPartNode)
                                {
