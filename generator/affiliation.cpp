...
CountriesFilesIndexAffiliation::Box MakeBox(m2::RectD const & rect) { return {rect.LeftBottom(), rect.RightTop()}; }
  ...
  std::vector<m2::RectD> const & net)
{
  ...
  }
  {
    ...
    {
      pool.SubmitWork(
        [&, countryPtr{pair.first}, rects{std::move(pair.second)}]() mutable
        {
...
std::vector<std::string> SingleAffiliation::GetAffiliations(FeatureBuilder const &) const { return {m_filename}; }

bool SingleAffiliation::HasCountryByName(std::string const & name) const { return name == m_filename; }

std::vector<std::string> SingleAffiliation::GetAffiliations(m2::PointD const &) const { return {m_filename}; }
}  // namespace feature
