...
public:
  ...
  virtual std::string ToString(regions::NodePath const & path) const = 0;
};
...
public:
  ...
  explicit JsonPolicy(bool extendedOutput = false) : m_extendedOutput(extendedOutput) {}
  JsonPolicy & SetRealPrecision(int32_t precision);
  std::string ToString(regions::NodePath const & path) const override;
