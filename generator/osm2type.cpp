...
string DetermineSurfaceAndHighwayType(OsmElement * p)
{
  ...
  {
    // TODO(@pastk): convert non-segregated cycleways (incl. default and foot=yes/designated)
    // into a footway or path (surface dependent) with bicycle=yes - to have hw=cycleway for dedicated ones only
  ...
  else if (highway == kFootway || highway == kPath)
  {
    ...
    {
      // A good quality path without explicit access tags is bicycle friendly.
      if (newType == kFootway && !p->HasTag("bicycle") && !p->HasTag("foot"))
        p->AddTag("bicycle", "yes");
