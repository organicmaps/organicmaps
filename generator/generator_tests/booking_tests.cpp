...
namespace
{
std::string const kFilePath = "booking_dataset_test.xml";
}  // namespace

UNIT_TEST(BookingDataset_SmokeTest)
{
  ScopedFile file(kFilePath, "" /* contents */);
  generator::BookingDataset data(file.GetFullPath());
  TEST_EQUAL(data.GetStorage().Size(), 0, ());
}
...
UNIT_TEST(BookingDataset_ParseTest)
{
  ...
  generator::BookingDataset data(file.GetFullPath());
  TEST_EQUAL(data.GetStorage().Size(), 1, ());
}
...
UNIT_TEST(BookingDataset_ParseTest2)
{
  ...
  generator::BookingDataset data(file.GetFullPath());
  TEST_EQUAL(data.GetStorage().Size(), 10, ());
}
