...
namespace
{
std::string_view GetPostcode(FeatureType & ft) { return ft.GetMetadata(feature::Metadata::FMD_POSTCODE); }
}  // namespace
...
UNIT_CLASS_TEST(TestRawGenerator, Postcode_Relations)
{
  ...
  size_t count = 0;
  ForEachFeature(mwmName,
                 [&count](std::unique_ptr<FeatureType> ft)
                 {
...
UNIT_TEST(Place_CityRelations)
{
  ...
  for (size_t i = 0; i < std::size(arrFiles); ++i)
  {
    ...
    size_t count = 0;
    generator.ForEachFB(worldMwmName,
                        [&](feature::FeatureBuilder const & fb)
                        {
