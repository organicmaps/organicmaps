...
std::string DebugPrint(Type const & t) { return "T" + std::to_string(static_cast<int>(t)); }
...
m2::RectD GetLimitRect(NamedPoint const & p) { return m2::RectD(p.m_point, p.m_point); }

bool operator==(NamedPoint const & left, NamedPoint const & right) { return left.m_id == right.m_id; }
...
void Sort(std::vector<ClusterT> & data)
{
  ...
  std::sort(std::begin(data), std::end(data),
            [](ClusterT const & l, ClusterT const & r)
            {
              TEST(!l.empty(), ());
              TEST(!r.empty(), ());
