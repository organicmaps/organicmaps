...
Batcher::~Batcher() { m_buckets.clear(); }
...
void Batcher::StartSession(TFlushFn && flusher) { m_flushInterface = std::move(flusher); }
...
void Batcher::SetBatcherHash(uint64_t batcherHash) { m_batcherHash = batcherHash; }
...
void Batcher::Flush(ref_ptr<GraphicsContext> context)
{
  ...
  std::for_each(m_buckets.begin(), m_buckets.end(),
                [this, context](TBuckets::value_type & bucket)
                {
...
Batcher * BatcherFactory::GetNew() const { return new Batcher(m_indexBufferSize, m_vertexBufferSize); }
...
SessionGuard::~SessionGuard() { m_batcher.EndSession(m_context); }
}  // namespace dp
