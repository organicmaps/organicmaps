...
void UniversalBatch::SetCanDivideStreams(bool canDivide) { m_canDivideStreams = canDivide; }

bool UniversalBatch::CanDivideStreams() const { return m_canDivideStreams; }

void UniversalBatch::SetVertexStride(uint8_t vertexStride) { m_vertexStride = vertexStride; }
...
void UniversalBatch::SubmitIndices(ref_ptr<GraphicsContext> context) { m_callbacks.SubmitIndices(context); }

uint32_t UniversalBatch::GetAvailableVertexCount() const { return m_callbacks.GetAvailableVertexCount(); }

uint32_t UniversalBatch::GetAvailableIndexCount() const { return m_callbacks.GetAvailableIndexCount(); }

void UniversalBatch::ChangeBuffer(ref_ptr<GraphicsContext> context) const { return m_callbacks.ChangeBuffer(context); }

uint8_t UniversalBatch::GetVertexStride() const { return m_vertexStride; }
...
bool FanStripHelper::IsFullUploaded() const { return m_isFullUploaded; }

uint32_t FanStripHelper::VtoICount(uint32_t vCount) const { return 3 * (vCount - 2); }

uint32_t FanStripHelper::ItoVCount(uint32_t iCount) const { return iCount / 3 + 2; }

uint32_t FanStripHelper::AlignVCount(uint32_t vCount) const { return vCount; }

uint32_t FanStripHelper::AlignICount(uint32_t iCount) const { return iCount - iCount % 3; }
...
TriangleFanBatch::TriangleFanBatch(BatchCallbacks & callbacks)
  : TBase(callbacks)
{}
...
uint32_t TriangleListOfStripBatch::AlignVCount(uint32_t vCount) const { return vCount - vCount % GetVertexStride(); }
