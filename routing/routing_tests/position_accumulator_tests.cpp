...
UNIT_CLASS_TEST(PositionAccumulator, Smoke)
{
  ...
  TEST(base::AlmostEqualAbs(GetTrackLengthMForTesting(), 11.13, kEpsMeters), (GetTrackLengthMForTesting()));
}
...
UNIT_CLASS_TEST(PositionAccumulator, GoodSegment)
{
  ...
  TEST_EQUAL(GetPointsForTesting().size(), 2, ());
  TEST(m2::AlmostEqualAbs(GetDirection(), m2::PointD(9 * kStepShortButValid, 0.0), kEps), (GetDirection()));
  TEST(base::AlmostEqualAbs(GetTrackLengthMForTesting(), 10.02, kEpsMeters), (GetTrackLengthMForTesting()));
  ...
  TEST(base::AlmostEqualAbs(GetTrackLengthMForTesting(), 77.92, kEpsMeters), (GetTrackLengthMForTesting()));
}
