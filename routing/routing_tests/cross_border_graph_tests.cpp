...
void TestEqualSegments(CrossBorderSegments const & s1, CrossBorderSegments const & s2)
{
  ...
  for (auto const & [segId1, data1] : s1)
  {
    ...
    TEST(base::AlmostEqualAbs(data1.m_start.m_point.GetLatLon(), data2.m_start.m_point.GetLatLon(), epsCoord), ());
    TEST(base::AlmostEqualAbs(data1.m_end.m_point.GetLatLon(), data2.m_end.m_point.GetLatLon(), epsCoord), ());
