...
void FillSubroutesInfo(Route & route, vector<turns::TurnItem> const & turns /* = kTestTurnsReachOnly */)
{
  ...
  route.SetSubroteAttrs(
    vector<Route::SubrouteAttrs>({Route::SubrouteAttrs(junctions.front(), junctions.back(), 0, kTestSegments.size())}));
}
  ...
  GetPlatform().RunTask(
    ...
    [&alongTimedSignal, this, &counter]()
    {
      ...
      m_session->SetRoutingCallbacks([&alongTimedSignal](Route const &, RouterResultCode)
                                     { alongTimedSignal.Signal(); }, nullptr /* rebuildReadyCallback */,
    ...
    GetPlatform().RunTask(
      ...
      [&oppositeTimedSignal, &info, this]()
      {
        ...
        m_session->SetRoutingCallbacks([&oppositeTimedSignal](Route const &, RouterResultCode)
                                       { oppositeTimedSignal.Signal(); }, nullptr /* rebuildReadyCallback */,
                                       nullptr /* needMoreMapsCallback */, nullptr /* removeRouteCallback */);
        {
          m_session->BuildRoute(Checkpoints(kTestRoute.front(), kTestRoute.back()), RouterDelegate::kNoTimeout);
        }
...
UNIT_CLASS_TEST(AsyncGuiThreadTestWithRoutingSession, TestRouteRebuildingMovingToRoute)
{
  ...
  // Going starting far from the route and moving to the route but rebuild flag still is set.
  {
    ...
    GetPlatform().RunTask(Platform::Thread::Gui,
                          [&checkTimedSignalAway, &info, this]()
                          {
...
UNIT_CLASS_TEST(AsyncGuiThreadTestWithRoutingSession, TestFollowRouteFlagPersistence)
{
  ...
  GetPlatform().RunTask(
    Platform::Thread::Gui,
    [&oppositeTimedSignal, &info, this, &counter]()
    {
...
UNIT_CLASS_TEST(AsyncGuiThreadTestWithRoutingSession, TestFollowRoutePercentTest)
{
  ...
  GetPlatform().RunTask(
    ...
    [&checkTimedSignal, this]
    {
      ...
      info.m_latitude = 1.;
      m_session->OnLocationPositionChanged(info);
      TEST(base::AlmostEqualAbs(m_session->GetCompletionPercent(), 0., 0.5), (m_session->GetCompletionPercent()));

      info.m_longitude = 0.;
      info.m_latitude = 2.;
      m_session->OnLocationPositionChanged(info);
      TEST(base::AlmostEqualAbs(m_session->GetCompletionPercent(), 33.3, 0.5), (m_session->GetCompletionPercent()));

      info.m_longitude = 0.;
      info.m_latitude = 3.;
      m_session->OnLocationPositionChanged(info);
      TEST(base::AlmostEqualAbs(m_session->GetCompletionPercent(), 66.6, 0.5), (m_session->GetCompletionPercent()));

      info.m_longitude = 0.;
      info.m_latitude = 3.99;
      m_session->OnLocationPositionChanged(info);
      TEST(base::AlmostEqualAbs(m_session->GetCompletionPercent(), 100., 0.5), (m_session->GetCompletionPercent()));
      checkTimedSignal.Signal();
    });
  TEST(checkTimedSignal.WaitUntil(steady_clock::now() + kRouteBuildingMaxDuration), ("Route checking timeout."));
}
...
UNIT_CLASS_TEST(AsyncGuiThreadTestWithRoutingSession, TestRouteRebuildingError)
{
  ...
  // Building a route.
  {
    ...
    GetPlatform().RunTask(
      Platform::Thread::Gui,
      [this, &kRoute, &buildTimedSignal]()
      {
        m_session->SetRoutingCallbacks([&buildTimedSignal](Route const &, RouterResultCode)
                                       { buildTimedSignal.Signal(); }, nullptr /* rebuildReadyCallback */,
                                       nullptr /* needMoreMapsCallback */, nullptr /* removeRouteCallback */);
