...
{
  connector.ForEachEnter(
    [&connector](uint32_t, Segment const & enter)
    {
  ...
  connector.ForEachExit(
    [&connector](uint32_t, Segment const & exit)
    {
...
void TestOneWayEnter(CrossMwmId const & crossMwmId)
{
  ...
  TEST(
    !test.connector.IsTransition(Segment(kTestMwmId, featureId, segmentIdx, true /* forward */), true /* isOutgoing */),
    ());
  TEST(
    test.connector.IsTransition(Segment(kTestMwmId, featureId, segmentIdx, true /* forward */), false /* isOutgoing */),
    ());
...
void TestTwoWayEnter(CrossMwmId const & crossMwmId)
{
  ...
  TEST(
    ...
    ());
  TEST(
    test.connector.IsTransition(Segment(kTestMwmId, featureId, segmentIdx, false /* forward */), true /* isOutgoing */),
    ());
  TEST(!test.connector.IsTransition(Segment(kTestMwmId, featureId, segmentIdx, false /* forward */),
                                    false /* isOutgoing */),
       ());
...
void GetCrossMwmId(uint32_t i, base::GeoObjectId & id) { id = base::GeoObjectId(base::MakeOsmWay(10 * i)); }
...
void TestWeightsSerialization()
{
  ...
  vector<uint8_t> buffer;
  {
    ...
    size_t weightIdx = 0;
    builder.FillWeights(
      [&](Segment const & enter, Segment const & exit)
      {
