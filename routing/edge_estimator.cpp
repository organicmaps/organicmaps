...
double GetCarClimbPenalty(EdgeEstimator::Purpose, double, geometry::Altitude) { return 1.0; }
    ...
    return CalcClimbSegment(purpose, segment, road,
                            [purpose, this](double speedMpS, double tangent, geometry::Altitude altitude)
                            {
                              ...
                              else
                              {
                                if (factor > 1)
                                {
                                  // Calculate uphill speed according to the average bicycle speed, because "good-roads"
                                  // like residential, secondary, cycleway are "equal-low-speed" uphill and road type
                                  // doesn't matter.
                                  ...
                                  if (speedMpS > upperBound)
                                  {
                                    ...
                                    speedMpS =
                                      upperBound + (purpose == Purpose::Weight ? speedMpS / (10 * avgBicycleSpeed) : 0);
                                  }
                                }
                                else
                                  speedMpS /= factor;
