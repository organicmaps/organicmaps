...
OsrmRouter::OsrmRouter(Index const * index, CountryFileFnT const & fn)
  : m_pIndex(index), m_indexManager(fn), m_isFinalChanged(false),
    m_requestCancel(false), m_additionalFeatures(false)
{
...
{
  ASSERT(routingMapping.get()!=nullptr, ());
  auto nSegs = routingMapping->m_segMapping.GetSegmentsRange(node_id);

  ASSERT_GREATER(nSegs.second, 0, ());

  OsrmFtSegMapping::FtSeg seg;
  if (use_start)
    routingMapping->m_segMapping.GetSegmentByIndex(nSegs.first, seg);
  else
...
{
  taskNode.m_node.forward_node_id = nodeId;
  taskNode.m_node.reverse_node_id = nodeId;
...
size_t OsrmRouter::FindNextMwmNode(RoutingMappingPtrT const & startMapping, size_t startId, RoutingMappingPtrT const & targetMapping)
{
  ...
  CrossRoutingContext const & targetContext = targetMapping->m_dataFacade.getRoutingContext();
...
OsrmRouter::ResultCode OsrmRouter::MakeRouteFromCrossesPath(CheckedPathT const & path, Route & route)
{
  ...
  for (RoutePathCross const & cross: path)
  {
    ...
    if (cross.startNode.m_seg.m_fid == OsrmFtSegMapping::FtSeg::INVALID_FID)
    {
      ...
      startTask[1].m_node.forward_node_id = INVALID_NODE_ID;
    }
    ...
    if (cross.targetNode.m_seg.m_fid == OsrmFtSegMapping::FtSeg::INVALID_FID)
    {
      ...
      targetTask[1].m_node.forward_node_id = INVALID_NODE_ID;
    }
  ...
  else //4.2 Different mwm case
  {
    ...
    if (crossTasks.size())
    {
      while (getPathWeight(crossTasks.top())<finalWeight)
      {
        ...
        ASSERT(currentMapping->IsValid(), ("We have what we test!"));

        MappingGuard currentMappingGuard(currentMapping);
        UNUSED_VALUE(currentMappingGuard);
        CrossRoutingContext const & currentContext = currentMapping->m_dataFacade.getRoutingContext();
