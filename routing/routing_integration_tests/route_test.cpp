...
UNIT_TEST(USALosAnglesAriaTwentyninePalmsHighwayTimeTest)
{
  TRouteResult const routeResult = CalculateRoute(
    GetVehicleComponents(VehicleType::Car), FromLatLon(34.0739, -115.3212), {0.0, 0.0}, FromLatLon(34.0928, -115.5930));
  RouterResultCode const result = routeResult.second;
  TEST_EQUAL(result, RouterResultCode::NoError, ());
  TEST(routeResult.first, ());
  Route const & route = *routeResult.first;
...
{
  TRouteResult const routeResult = CalculateRoute(
    GetVehicleComponents(VehicleType::Car), FromLatLon(53.08279, 25.30036), {0.0, 0.0}, FromLatLon(53.09443, 25.34356));
  RouterResultCode const result = routeResult.second;
  TEST_EQUAL(result, RouterResultCode::NoError, ());
}
...
{
  TRouteResult const routeResult = CalculateRoute(
    GetVehicleComponents(VehicleType::Car), FromLatLon(53.09422, 25.34411), {0.0, 0.0}, FromLatLon(53.09271, 25.3467));
  RouterResultCode const result = routeResult.second;
  TEST_EQUAL(result, RouterResultCode::NoError, ());
}
