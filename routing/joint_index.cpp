...
void JointIndex::Build(RoadIndex const & roadIndex, uint32_t numJoints)
{
  ...
  // 2, 5, 3, 4, 2, 3, 0
  roadIndex.ForEachRoad(
    [this, numJoints](uint32_t /* featureId */, RoadJointIds const & road)
    {
      road.ForEachJoint(
        [this, numJoints](uint32_t /* pointId */, Joint::Id jointId)
        {
  ...
  // 0, 2, 7, 10, 14, 16, 19
  roadIndex.ForEachRoad(
    [this](uint32_t featureId, RoadJointIds const & road)
    {
      road.ForEachJoint(
        [this, featureId](uint32_t pointId, Joint::Id jointId)
        {
