...
namespace
{
bool IsLocalBuild() { return !FLAGS_routes_file.empty() && FLAGS_api_name.empty() && FLAGS_api_token.empty(); }

bool IsApiBuild() { return !FLAGS_routes_file.empty() && !FLAGS_api_name.empty() && !FLAGS_api_token.empty(); }

void CheckDirExistence(std::string const & dir) { CHECK(Platform::IsDirectory(dir), ("Can not find directory:", dir)); }
}  // namespace
