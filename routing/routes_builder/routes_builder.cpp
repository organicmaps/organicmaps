...
void RoutesBuilder::Processor::InitRouter(VehicleType type)
{
  ...
  m_router =
    std::make_unique<IndexRouter>(type, loadAltitudes, *m_cpg.lock(), countryFileGetter, getMwmRectByName, m_numMwmIds,
                                  MakeNumMwmTree(*m_numMwmIds, *m_cig.lock()), *m_trafficCache, *m_dataSource);
}
