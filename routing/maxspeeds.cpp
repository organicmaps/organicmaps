...
namespace routing
{
bool Maxspeeds::IsEmpty() const { return m_forwardMaxspeedsTable.size() == 0 && m_bidirectionalMaxspeeds.empty(); }
...
Maxspeed Maxspeeds::GetMaxspeed(uint32_t fid) const
{
  ...
  auto const range =
    std::equal_range(m_bidirectionalMaxspeeds.cbegin(), m_bidirectionalMaxspeeds.cend(), fid, FeatureMaxspeed::Less());
