...
std::string NotificationManager::GenerateTurnText(uint32_t distanceUnits, uint8_t exitNum,
    bool useThenInsteadOfDistance, TurnItem const & turn, RouteSegment::RoadNameInfo const & nextStreetInfo) const
{
  auto const lengthUnits = m_settings.GetLengthUnits();

  if (turn.m_turn != CarDirection::None)
    return m_getTtsText.GetTurnNotification({distanceUnits, exitNum, useThenInsteadOfDistance, turn.m_turn, lengthUnits, nextStreetInfo});
                                                    ...
                                                    RouteSegment::RoadNameInfo const & nextStreetInfo)
{
  ...
  std::string secondNotification = GenerateTurnText(
      m_settings.ConvertMetersToUnits(distBetweenTurnsMeters), secondTurn.m_turnItem.m_exitNum,
                                                        ...
                                                        RouteSegment::RoadNameInfo const & nextStreetInfo)
{
      ...
      if (distanceToTurnMeters < startPronounceDistMeters)
      {
        ...
        else
        {
          ...
          return GenerateTurnText(roundedDistToPronounceUnits, turn.m_exitNum,
                                  false /* useThenInsteadOfDistance */, turn, nextStreetInfo);
        }
      }
    }
    else
      ...
      distanceToTurnMeters < distanceToPronounceNotificationM)
  {
    ...
    FastForwardFirstTurnNotification();
    return GenerateTurnText(0 /* distMeters */, turn.m_exitNum, false /* useThenInsteadOfDistance */, turn, nextStreetInfo);
  }
  return {};
}
