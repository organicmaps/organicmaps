                                                        ...
                                                        IndexGraphStarter & starter)
{
  ...
  {
    auto it = find_if(linksFrom.begin(), linksFrom.end(),
                      [&link](ConnectionToOsm const & cur)
                      {
                                                             ...
                                                             vector<Segment> & subroute)
{
...

  {
    ...
    auto const result =
      AlgoT().FindPathBidirectionalEx(params,
                                      [&](RoutingResultT && route)
                                      {
    ...
    public:
      HighwayCategoryChecker(IndexGraphStarter const & starter)
        : m_starter(starter)
      {
...
void IndexRouter::PointsOnEdgesSnapping::FillDeadEndsCache(m2::PointD const & point)
{
  ...
  auto closestRoads =
    m_router.m_roadGraph.FindRoads(rect,
                                   [this](FeatureID const & fid)
                                   {
  ...
  RoadsToNearestEdges(
    point, closestRoads,
    [this](EdgeProjectionT const & proj)
    {
                                                       ...
                                                       bool & bestSegmentIsAlmostCodirectional)
{
  ...
  sort(closestRoads.begin(), closestRoads.end(),
       [&checkpoint](RoadInfoT const & lhs, RoadInfoT const & rhs)
       {
