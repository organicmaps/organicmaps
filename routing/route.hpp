...
public:
  ...
  struct RoadNameInfo
  {
    ...
    RoadNameInfo(std::string name, std::string ref, std::string junction_ref,
                 std::string destination_ref, std::string destination, bool isLink)
      : m_name(std::move(name))
      , m_destination_ref(std::move(destination_ref))
      , m_junction_ref(std::move(junction_ref))
    ...
    bool empty() const
    {
      return m_name.empty() && m_ref.empty() && m_junction_ref.empty() &&
             m_destination_ref.empty() && m_destination.empty();
    }
