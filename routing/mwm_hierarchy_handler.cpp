...
bool IsInSet(CountrySetT const & theSet, std::string const & country) { return theSet.find(country) != theSet.end(); }
}  // namespace
...
std::string MwmHierarchyHandler::GetMwmName(NumMwmId mwmId) const { return m_numMwmIds->GetFile(mwmId).GetName(); }
