...
void ExpandFake(Graph::EdgeVector & path, Graph::EdgeVector::iterator edgeIt, DataSource const & dataSource, Graph & g)
{
  ...
  auto it = find_if(begin(edges), end(edges),
                    [&edgeIt](Graph::Edge const & real)
                    {
  ...
  if (it == end(edges))
  {
    it = find_if(begin(edges), end(edges),
                 [&edgeIt, &startIsFake](Graph::Edge const & real)
                 {
                        ...
                        uint32_t negativeOffset, bool keepEnds)
{
  ...
  if (distance(start, stop) > 1)
  {
    ...
    to =
      CutOffset(reverse_iterator<InputIterator>(stop), reverse_iterator<InputIterator>(start), negativeOffset, keepEnds)
        .base();
  }
...
size_t constexpr GetOptimalBatchSize()
{
  ...
  size_t constexpr b = base::LCM(sizeof(IRoadGraph::EdgeVector), kCacheLineSize) / sizeof(IRoadGraph::EdgeVector);
  return base::LCM(a, b);
}
}  // namespace
