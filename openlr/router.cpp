...
bool Router::FindPath(Path & path)
{
  ...
  while (!queue.empty())
  {
    ...
    ForEachEdge(u, true /* outgoing */, m_points[stage].m_lfrcnp,
                [&](routing::Edge const & edge)
                {
                  ...
                  if (NeedToCheckBearing(v, vd))
                  {
                    ...
                    auto const p =
                      PointAtSegment(edge.GetStartJunction().GetPoint(), edge.GetEndJunction().GetPoint(), delta);
...
bool Router::NearNextStage(Vertex const & u, double pi) const { return u.m_stage < m_pivots.size() && pi < kEps; }
...
{
  ForEachEdge(u, outgoing, restriction,
              [&fn](routing::Edge const & edge)
              {
                if (!edge.IsFake())
                  fn(edge);
              });
...
void Router::ForEachNonFakeClosestEdge(Vertex const & u, FunctionalRoadClass const restriction, Fn && fn)
{
  std::vector<std::pair<routing::Edge, geometry::PointWithAltitude>> vicinity;
  m_graph.FindClosestEdges(
    mercator::RectByCenterXYAndSizeInMeters(u.m_junction.GetPoint(), routing::FeaturesRoadGraph::kClosestEdgesRadiusM),
    kMaxRoadCandidates, vicinity);
...
bool Router::ReconstructPath(std::vector<Edge> & edges, Path & path)
{
  ...
  ForStagePrefix(edges.begin(), edges.end(), 0,
                 [&](EdgeIt e)
                 {
  ...
  ForStagePrefix(edges.rbegin(), edges.rend(), m_points.size() - 2 /* stage */,
                 [&](EdgeItRev e)
                 {
