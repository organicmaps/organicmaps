...
UNIT_TEST(HttpClient_ReceivedFile_InvalidPath)
{
  ...
  // On Qt (Linux/Windows), the file open failure is detected and reported as a write error.
  // On Apple, NSURLSession writes atomically and may report a different error.
  // The key invariant: we must NOT silently report success with no file created.
#if defined(OMIM_OS_LINUX) || defined(OMIM_OS_WINDOWS)
  TEST(!result.m_success, ());
  TEST_EQUAL(result.m_errorCode, -2, ());  // kWriteException
#endif
}
