...
void EraseRandom(strings::UniString & uni)
{
  ...
  std::uniform_int_distribution<size_t> dis(1, uni.size() - 1);
...
void SwapRandom(strings::UniString & uni)
{
  ...
  std::uniform_int_distribution<size_t> dis(1, uni.size() - 2);
  auto const index = dis(g_rng);
  std::swap(uni[index], uni[index + 1]);
}
...
void AddMisprints(std::string & str)
{
...
std::map<std::string, std::vector<std::string>> const kStreetSynonyms = {
    {"улица", {"ул", "у"}},
...
void ModifyStreet(std::string & str)
{
  auto tokens = strings::Tokenize<std::string>(str, " -&");
  str.clear();

  auto const isStreetSynonym = [](std::string const & s) { return kStreetSynonyms.find(s) != kStreetSynonyms.end(); };

  auto const synonymIt = std::find_if(tokens.begin(), tokens.end(), isStreetSynonym);
  if (synonymIt != tokens.end() && std::find_if(synonymIt + 1, tokens.end(), isStreetSynonym) == tokens.end())
  {
    ...
    if (GetRandomBool())
    {
      if (GetRandomBool())
      {
        ...
        std::uniform_int_distribution<size_t> dis(0, synonyms.size() - 1);
...
void ModifyHouse(uint8_t lang, std::string & str)
{
  if (str.empty())
    return;
  ...
  {
    std::uniform_int_distribution<size_t> dis(0, 4);
...
m2::PointD GenerateNearbyPosition(m2::PointD const & point)
{
  ...
  std::uniform_real_distribution<double> dis(-maxDistance, maxDistance);
  return mercator::GetSmPoint(point, dis(g_rng) /* dX */, dis(g_rng) /* dY */);
}

m2::RectD GenerateNearbyViewport(m2::PointD const & point)
{
  std::uniform_real_distribution<double> dis(FLAGS_min_viewport_size, FLAGS_max_viewport_size);
  return mercator::RectByCenterXYAndSizeInMeters(GenerateNearbyPosition(point), dis(g_rng));
}

bool GetBuildingInfo(FeatureType & ft, search::ReverseGeocoder const & coder, std::string & street)
{
...
bool GetCafeInfo(FeatureType & ft, search::ReverseGeocoder const & coder, std::string & street, uint32_t & cafeType,
...
std::string ModifyAddress(std::string street, std::string house, uint8_t lang)
{
...
std::string CombineRandomly(std::string const & mandatory, std::string const & optional)
{
...
void ModifyCafe(std::string const & name, std::string const & type, std::string & out)
{
  out = FLAGS_add_cafe_type ? CombineRandomly(name, type) : name;
  AddMisprints(out);
}

std::string_view GetLocalizedCafeType(std::unordered_map<uint32_t, StringUtf8Multilang> const & typesTranslations,
...
std::optional<Sample> GenerateRequest(FeatureType & ft, search::ReverseGeocoder const & coder,
                                      std::unordered_map<uint32_t, StringUtf8Multilang> const & typesTranslations,
...
std::unordered_map<uint32_t, StringUtf8Multilang> ParseStrings()
{
  ...
  std::unordered_map<uint32_t, StringUtf8Multilang> typesTranslations;
  while (s.good())
  {
    std::getline(s, line);
    strings::Trim(line);
...
int main(int argc, char * argv[])
{
  ...
  for (auto const & mwmInfo : mwmInfos)
  {
    ...
    auto generate = [&](auto const & checker, RequestType type, std::ofstream & out)
    {
      ...
      std::shuffle(fids.begin(), fids.end(), g_rng);
