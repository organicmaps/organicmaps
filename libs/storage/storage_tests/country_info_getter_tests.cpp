...
UNIT_TEST(CountryInfoGetter_RegionRects)
{
  ...
  {
    if (countries[i].HasExtendedRect())
      continue;
...
UNIT_TEST(CountryInfoGetter_ExtendedRect_ReadWriteRoundtrip)
{
  ...
  TestCase const kTestCases[] = {
      ...
      {"US_Alaska", {169.78, 50.00, 230.03, 72.80}},
      {"Kiribati", {168.84, -12.95, 210.81, 7.03}},
...
UNIT_TEST(CountryInfoGetter_CalcLimitRect_TwoGrouping)
{
  // Verify CalcLimitRect two-grouping works with extended rects.
  // Use CountryInfoGetterForTesting with synthetic data simulating post-generator-fix rects.
  CountryInfoGetterForTesting getter;
  getter.AddCountry(CountryDef("NZ_Auckland", m2::RectD(168.0, -38.0, 180.0, -34.0)));
  getter.AddCountry(CountryDef("NZ_Wellington", m2::RectD(168.0, -42.0, 180.0, -38.0)));
  getter.AddCountry(CountryDef("NZ_Canterbury", m2::RectD(166.0, -45.5, 186.0, -28.0)));  // Extended
  getter.AddCountry(CountryDef("NZ_Southland", m2::RectD(162.0, -47.0, 180.0, -44.0)));
  getter.AddCountry(CountryDef("Tokelau", m2::RectD(-175.0, -10.0, -167.0, -5.0)));  // Western

  // Simulate CalcLimitRect logic: naive union, then two-grouping if > 180°.
  m2::RectD boundingBox;
  std::vector<m2::RectD> rects;
  for (auto const & c : getter.GetCountries())
  {
    rects.push_back(c.m_rect);
    boundingBox.Add(c.m_rect);
  }

  // Naive union should be > 180° because of Tokelau + extended Canterbury.
  TEST_GREATER(boundingBox.SizeX(), 180.0, ());

  auto const compacted = mercator::CompactRectsAcrossAntimeridian(rects);

  // East grouping: Tokelau shifted to [185, 193], rest stays → [162, 193] = 31°
  TEST_LESS(compacted.SizeX(), 40.0, ("Two-grouping should produce compact NZ rect"));
  LOG(LINFO, ("Compacted NZ rect:", compacted, "width:", compacted.SizeX()));
}
