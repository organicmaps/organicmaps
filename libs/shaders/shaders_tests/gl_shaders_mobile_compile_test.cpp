...
UNIT_TEST(MALI_MobileCompileShaders_Test)
{
  ...
  QString const compiler = GetCompilerPath(kMaliCompilerOpenGLES3Dir + "/malisc");

  // Each driver library that malisc loads from its own openglessl directory is a separate compiler. Without -c and -r
  // malisc targets the newest GPU core and revision of the driver, which affect only the generated code, not whether a
  // shader compiles.
  std::string const driversDir = base::JoinPath(kCompilersDir, kMaliCompilerOpenGLES3Dir, "openglessl");
  Platform::FilesList drivers;
  Platform::GetFilesByRegExp(driversDir, R"(^libMali-.+\.(so|dylib)$)", drivers);
  // 12 drivers are bundled for macOS and 14 for Linux: a bundle that loses some must not silently reduce the coverage.
  TEST_GREATER_OR_EQUAL(drivers.size(), 12u, (driversDir, drivers));
