...
UNIT_TEST(MapStyleModeSettingsRoundTrip)
{
  for (auto const mode : {MapStyleMode::Default, MapStyleMode::Outdoors, MapStyleMode::Cycling})
  {
    auto const parsed = MapStyleModeFromSettings(MapStyleModeToString(mode));
    TEST(parsed && *parsed == mode, (MapStyleModeToString(mode)));
  }
  TEST(!MapStyleModeFromSettings("unknown"), ());
}
