...
void RoutingManager::ReplaceRoutePoints(std::vector<RouteMarkData> points, bool optimize)
{
  ...
  // The group was just cleared and the count is capped above, so the layout cannot run out of capacity.
  CHECK(layout.AddRoutePoint(std::move(points.front())), ());
  CHECK(layout.AddRoutePoint(std::move(points.back())), ());
  for (size_t i = 1; i + 1 < points.size(); ++i)
  {
    auto const * added = layout.AddRoutePoint(std::move(points[i]));
    CHECK(added != nullptr, ());
    // Place each stop as it arrives, like AddRoutePoint() does, so the itinerary is optimized incrementally.
    if (optimize)
      ReorderIntermediatePoints(layout, added);
  }
}
