...
void Framework::CreateDrapeEngine(ref_ptr<dp::GraphicsContextFactory> contextFactory, DrapeCreationParams && params)
{
  ...
  auto getMwmHandleByIdFn = [this](MwmSet::MwmId const & mwmId)
  { return m_featuresFetcher.GetDataSource().GetMwmHandleById(mwmId); };
  ...
  df::DrapeEngine::Params p(
      ...
      df::MapDataProvider(std::move(idReadFn), std::move(featureReadFn), std::move(getMwmHandleByIdFn),
