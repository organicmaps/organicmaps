...
void GpsPointToGpsInfo(GpsPoint const gpsPoint, location::GpsInfo & gpsInfo)
{
  gpsInfo = {};
  gpsInfo.m_source = location::TLocationSource::EAppleNative;
...
int main(int argc, char * argv[])
{
  ...
  auto const extrapolationNumber = static_cast<size_t>(extrapolation::Extrapolator::kMaxExtrapolationTimeMs /
                                                       extrapolation::Extrapolator::kExtrapolationPeriodMs);
  ...
  for (auto const & t : tracks)
  {
    ...
    routing::FollowedPolyline followedPoly(poly.Begin(), poly.End());
    CHECK(followedPoly.IsValid(), ());

    // For each track point except for the first one some extrapolations will be calculated.
    for (size_t i = 1; i < t.size(); ++i)
    {
      location::GpsInfo info1;
      GpsPointToGpsInfo(t[i - 1], info1);
      location::GpsInfo info2;
      GpsPointToGpsInfo(t[i], info2);

      if (!extrapolation::AreCoordsGoodForExtrapolation(info1, info2))
        break;

      std::vector<double> onePointDeviations;
      std::vector<double> onePointDeviationsSquared;
      bool projFound = true;
      for (size_t timeMs = extrapolation::Extrapolator::kExtrapolationPeriodMs;
           timeMs <= extrapolation::Extrapolator::kMaxExtrapolationTimeMs;
           timeMs += extrapolation::Extrapolator::kExtrapolationPeriodMs)
      {
        location::GpsInfo const extrapolated = extrapolation::LinearExtrapolation(info1, info2, timeMs);
        ...
        auto const & iter = followedPoly.GetClosestProjectionInInterval(
            posSquare, [&extrapolatedMerc](routing::FollowedPolyline::Iter const & it)
        {
          ASSERT(it.IsValid(), ());
  ...
  for (size_t i = 0; i < extrapolationNumber; ++i)
  {
    ...
    LOG(LINFO, ("Extrapolation", i + 1, ",", extrapolation::Extrapolator::kExtrapolationPeriodMs * (i + 1),
