...
public:
  ...
  /// existing endpoint instead. A new stop is dropped at capacity, except a My Position one, which moves the existing
  /// My Position mark. optimize places only the new stop at the predicted position (needs both endpoints, ignored in
  /// Ruler mode, still applied while following).
  void AddRoutePoint(RouteMarkData && markData, bool optimize);
  /// Replaces an existing slot without optimization. If the target is gone, adds the point normally; type/index
  /// override markData.
  void ReplaceRoutePoint(RouteMarkType type, size_t intermediateIndex, RouteMarkData && markData);
  /// Reorders unpassed intermediate marks in place, keeping endpoints and passed stops. Returns whether the order
  /// changed; the caller rebuilds. No-op while following, in Ruler mode, or without both endpoints.
