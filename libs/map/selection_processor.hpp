...
public:
  ...
  struct TapFeatures
  {
    FeatureID m_poi;       ///< Best Point feature.
    FeatureID m_building;  ///< First building (area).
    FeatureID m_bestLine;  ///< Closest line feature.
    FeatureID m_bestArea;  ///< Closest non-building area feature.
    /// All line candidates within tap distance, sorted by distance (closest first).
  ...
  /// @param lineDistThresholdM  max distance in meters to accept a line feature.
  /// @param matcher  optional callback; if it returns true for a feature, that feature is returned
  ///   immediately as GetBest() with all other fields empty.
  TapFeatures FindFeaturesInRect(m2::PointD const & mercator, m2::RectD const & searchRect, double lineDistThresholdM,
