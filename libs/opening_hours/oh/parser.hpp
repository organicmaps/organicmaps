// parser.hpp – Self-contained C++23 OSM opening_hours parser
// ============================================================================
//
// C++ port of the Rust `opening-hours-syntax` crate from
// https://github.com/remi-dupre/opening-hours-rs (MIT OR Apache-2.0, see
// LICENSE-MIT and LICENSE-APACHE next to this file). The evaluation crate is
// ported in oh/eval.hpp; osmoh's app-facing facade lives in
// opening_hours/opening_hours.hpp.
//
...
{
  constexpr char const * tbl[] = {"",    "Jan", "Feb", "Mar", "Apr", "May", "Jun",
...
constexpr char const * time_event_str(TimeEvent e)
{
  constexpr char const * tbl[] = {"dawn", "sunrise", "sunset", "dusk"};
...
constexpr char const * rule_kind_str(RuleKind k)
{
  constexpr char const * tbl[] = {"open", "closed", "unknown"};
...
enum class HolidayKind : uint8_t
{
  Public,
  School
};

constexpr char const * holiday_kind_str(HolidayKind k)
{
  return k == HolidayKind::Public ? "PH" : "SH";
}

enum class WeekDayOffsetKind : uint8_t
{
  None,
  Next,
  Prev
...
struct ExtendedTime
{
  ...
  constexpr uint16_t mins_from_midnight() const { return static_cast<uint16_t>(hour) * 60 + minute; }
  ...
  constexpr auto operator<=>(ExtendedTime const &) const = default;

  std::string to_string() const { return fmt02(hour) + ":" + fmt02(minute); }
};
...
struct VariableTime
{
  ...
  auto operator<=>(VariableTime const &) const = default;
...
struct Time
{
  ...
  std::string to_string() const { return tag == Fixed ? fixed.to_string() : variable.to_string(); }
};
...
struct TimeSpan
{
  ...
  auto operator<=>(TimeSpan const &) const = default;
...
struct TimeSelector
{
  ...
  TimeSelector() : time{TimeSpan::fixed_range(ExtendedTime::midnight_00(), ExtendedTime::midnight_24())} {}
  explicit TimeSelector(std::vector<TimeSpan> t)
  {
    if (t.empty())
      time = {TimeSpan::fixed_range(ExtendedTime::midnight_00(), ExtendedTime::midnight_24())};
    else
      time = std::move(t);
  }
  ...
  {
    return time.size() == 1 &&
           time[0] == TimeSpan::fixed_range(ExtendedTime::midnight_00(), ExtendedTime::midnight_24());
  }

  bool operator==(TimeSelector const &) const = default;
...
struct YearRange
{
  ...
  auto operator<=>(YearRange const &) const = default;
...
struct WeekRange
{
  ...
  auto operator<=>(WeekRange const &) const = default;
...
struct DateOffset
{
  ...
  auto operator<=>(DateOffset const &) const = default;
...
struct Date
{
  ...
  static Date make_fixed(std::optional<uint16_t> y, Month m, uint8_t d) { return {DateKind::Fixed, y, m, d, {}, 0}; }
  ...
  static Date make_easter(std::optional<uint16_t> y) { return {DateKind::Easter, y, Month::Jan, 0, {}, 0}; }
  ...
  std::string to_string() const
  {
    ...
    switch (kind)
    {
    case DateKind::Fixed: s += std::string(month_short(month)) + " " + std::to_string(day); break;
    case DateKind::WeekdayInMonth:
      s += std::string(month_short(month)) + " " + weekday_short(weekday);
      if (nth != 0)
        s += "[" + std::to_string(nth) + "]";
      break;
    case DateKind::Easter: s += "easter"; break;
    }
    return s;
  }
};
...
enum class MonthdayRangeKind : uint8_t
{
  MonthOnly,
  DateRange
};
...
struct MonthdayRange
{
  ...
  auto operator<=>(MonthdayRange const &) const = default;
  ...
  std::string to_string() const
  {
    ...
    case MonthdayRangeKind::MonthOnly:
    {
      std::string s;
      if (month_year)
        s += std::to_string(*month_year);
      s += month_short(month_start);
      if (month_start != month_end)
        s += std::string("-") + month_short(month_end);
      return s;
    }
...
enum class WeekDayRangeKind : uint8_t
{
  FixedDays,
  Holiday
};
...
struct WeekDayRange
{
  ...
  auto operator<=>(WeekDayRange const &) const = default;
  ...
  std::string to_string() const
  {
    ...
    case WeekDayRangeKind::FixedDays:
      ...
      if (range_start != range_end)
        s += std::string("-") + weekday_short(range_end);
      if (nth_has_filter())
      {
        s += "[";
        bool first = true;
...
struct DaySelector
{
  ...
  bool is_empty() const { return year.empty() && monthday.empty() && week.empty() && weekday.empty(); }

  bool operator==(DaySelector const &) const = default;
  ...
  std::string to_string(bool force = false) const
  {
    ...
    if (force && is_empty())
      return "Mo-Su";
    std::string s;
    auto append_selector = [](std::string & out, auto const & vec)
    {
...
struct RuleSequence
{
  ...
  bool is_constant() const { return day_selector.is_empty() && time_selector.is_00_24(); }

  bool operator==(RuleSequence const &) const = default;
...
inline constexpr char const * kWeekdays[] = {"Mo", "Tu", "We", "Th", "Fr", "Sa", "Su"};

inline bool is_weekday_token(std::string_view s)
{
  for (auto w : kWeekdays)
    if (s == w)
      return true;
  return false;
}
...
inline std::optional<std::pair<char const *, size_t>> match_keyword(std::vector<char> const & chars, size_t pos)
{
  ...
  for (auto & [pattern, canonical] : kKeywords)
  {
    ...
    {
      if (static_cast<char>(std::tolower(static_cast<unsigned char>(chars[pos + k]))) != pattern[k])
      {
        ok = false;
        break;
      }
...
inline std::optional<size_t> match_spaced_dash_weekday(std::vector<char> const & chars, size_t pos)
{
  ...
  if (j + 1 < len)
  {
    ...
    if (ok('M', 'o') || ok('T', 'u') || ok('W', 'e') || ok('T', 'h') || ok('F', 'r') || ok('S', 'a') || ok('S', 'u') ||
        ok('D', 'i') || ok('M', 'i') || ok('D', 'o') || ok('S', 'o'))
      return j - pos;
  }
  return std::nullopt;
}

inline bool last_token_is_weekday(std::string const & result)
{
  auto t = result;
...
inline bool ends_with_time(std::string const & s)
{
  ...
  if (t.empty())
    return false;
  // A trailing '+' only counts after an actual time ("17:00+"): open-ended
  // year and date selectors ("2020+ Mo-Fr ...") end with '+' too and must not
  // trigger the missing-semicolon repair.
  if (t.back() == '+')
    t.pop_back();
  size_t n = t.size();
  if (n >= 3 && std::isdigit(static_cast<unsigned char>(t[n - 1])) &&
      std::isdigit(static_cast<unsigned char>(t[n - 2])) && t[n - 3] == ':')
    return true;
  return false;
}

inline bool should_insert_space_after_comma(std::string const & result)
{
  if (result.empty())
    return false;
  if (std::isdigit(static_cast<unsigned char>(result.back())))
    return true;
  auto t = result;
  while (!t.empty() && t.back() == ' ')
    t.pop_back();
...
// Convert AM/PM times.
inline std::optional<std::pair<std::string, size_t>> try_match_ampm(uint8_t const * bytes, size_t start, size_t len)
{
  size_t i = start;
  uint32_t h = bytes[i] - '0';
  ++i;
  if (i < len && std::isdigit(bytes[i]))
  {
    h = h * 10 + (bytes[i] - '0');
    ++i;
  }
  ...
  if (em != 'M' || (ap != 'A' && ap != 'P'))
    return std::nullopt;
  size_t end_pos = i + 2;
  if (end_pos < len && std::isalpha(bytes[end_pos]))
    return std::nullopt;
  uint32_t h24 = (ap == 'P') ? (h == 12 ? 12 : h + 12) : (h == 12 ? 0 : h);
  char buf[8];
  std::snprintf(buf, sizeof(buf), "%02u:%c%c", h24, m0, m1);
  return std::pair{std::string(buf), end_pos - start};
}

inline std::string pre_convert_ampm(std::string_view input)
{
  auto const * bytes = reinterpret_cast<uint8_t const *>(input.data());
...
inline std::optional<std::pair<std::string, size_t>> try_match_h_time(uint8_t const * bytes, size_t start, size_t len)
{
  size_t i = start;
  uint32_t h = bytes[i] - '0';
  ++i;
  ...
  if (i >= len || (bytes[i] != 'h' && bytes[i] != 'H'))
    return std::nullopt;
  ++i;
  if (i < len && std::isalpha(bytes[i]))
    return std::nullopt;
  char m0 = '0', m1 = '0';
  if (i + 1 < len && std::isdigit(bytes[i]) && std::isdigit(bytes[i + 1]))
  {
  ...
  if (m > 59 || (h == 24 && m > 0))
    return std::nullopt;
  char buf[8];
  std::snprintf(buf, sizeof(buf), "%02u:%c%c", h, m0, m1);
...
inline std::string pre_convert_dot_times(std::string_view input)
{
  ...
  for (size_t i = 1; i + 2 < len; ++i)
  {
    ...
    if (has_h0 && i >= 3 && std::isdigit(bytes[i - 3]))
      continue;
    if (!std::isdigit(bytes[i + 1]) || !std::isdigit(bytes[i + 2]))
      continue;
    if (i + 3 < len && (std::isdigit(bytes[i + 3]) || bytes[i + 3] == '.'))
      continue;
...
inline constexpr char const * kMonthNames[] = {"Jan", "Feb", "Mar", "Apr", "May", "Jun",
                                               "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"};
...
inline std::string post_insert_comment_colon(std::string_view input)
{
  ...
  while (true)
  {
    ...
    out += rest.substr(0, q2);
    out += '"';
    rest = rest.substr(q2 + 1);
    if (rest.starts_with(' ') && !rest.starts_with(" :"))
    {
      auto after = rest.substr(1);
      bool is_oh = after.starts_with("Mo") || after.starts_with("Tu") || after.starts_with("We") ||
                   after.starts_with("Th") || after.starts_with("Fr") || after.starts_with("Sa") ||
                   after.starts_with("Su") || after.starts_with("PH") || after.starts_with("SH") ||
...
inline std::string post_expand_date_comma_list(std::string_view input)
{
  ...
  for (size_t i = 0; i < len;)
  {
    ...
    if (i + 3 <= len && std::isupper(bytes[i]) && std::islower(bytes[i + 1]) && std::islower(bytes[i + 2]))
    {
      std::string_view cand{input.data() + i, 3};
      bool is_month = false;
      for (auto mn : kMonthNames)
        if (cand == mn)
        {
          is_month = true;
          break;
      ...
      if (is_month)
      {
        ...
        if (i < len && bytes[i] == ' ')
        {
          ...
          ++i;
          size_t day_start = i;
          while (i < len && std::isdigit(bytes[i]))
          {
            out += static_cast<char>(bytes[i]);
            ++i;
          }
          if (i > day_start)
          {
            ...
            {
              size_t ac = i + 1, j = ac;
              while (j < len && std::isdigit(bytes[j]))
                ++j;
              size_t dl = j - ac;
              if (dl >= 1 && dl <= 2 && (j >= len || bytes[j] != ':') && (j >= len || !std::isalpha(bytes[j])))
              {
                out += ',';
                out += month;
                out += ' ';
...
inline std::string normalize_input(std::string_view raw)
{
  ...
  while (i < len)
  {
    ...
    // Range separators are frequently pasted as typographic or full-width
    // dashes. Normalize the common variants accepted as a range/minus sign.
    static constexpr std::array<std::string_view, 10> kUnicodeRangeDashes = {
    ...
    {
      auto const remaining = std::string_view(chars.data() + i, len - i);
      auto const dash = std::ranges::find_if(
          kUnicodeRangeDashes, [&remaining](std::string_view value) { return remaining.starts_with(value); });
      if (dash != kUnicodeRangeDashes.end())
      {
        out += '-';
        i += dash->size();
        continue;
    ...
    if (i + 2 < len && static_cast<uint8_t>(chars[i]) == 0xEF && static_cast<uint8_t>(chars[i + 1]) == 0xBC &&
    ...
    if (c == ',' && i + 1 < len && chars[i + 1] != ' ' && should_insert_space_after_comma(out))
    {
      if (std::isalpha(static_cast<unsigned char>(chars[i + 1])) && match_keyword(chars, i + 1))
      {
        out += ',';
        out += ' ';
        ++i;
    ...
    if (c == '-' && !out.empty() && out.back() == ' ' && i + 1 < len && chars[i + 1] == ' ')
    {
  ...
  auto collapse = [](std::string & s, std::string const & from, std::string const & to)
  {
    size_t pos = 0;
...
private:
  ...
  bool match_str_ascii_ci(std::string_view s)
  {
    ...
    for (size_t i = 0; i < s.size(); ++i)
    {
      ...
      if (std::tolower(got) != std::tolower(expected))
        return false;
    }
    advance(s.size());
    return true;
  ...
  // "," with optional trailing spaces, for relaxed selector lists.
  bool match_comma_spaces()
  {
    if (!match_char(','))
      return false;
    skip_spaces();
  ...
  void set_fatal(std::string const & msg)
  {
    set_error(msg);
    fatal_ = true;
  }

  std::string_view substr_from(size_t start) const { return input_.substr(start, pos_ - start); }
  ...
  std::optional<std::vector<RuleSequence>> parse_opening_hours()
  {
    ...
    if (!first)
    {
      set_error("expected rule sequence");
      return std::nullopt;
    }
  ...
  std::optional<RuleSequence> parse_rule_sequence(RuleOperator op)
  {
    ...
    // Negative lookahead: not (optional_space (separator | end))
    {
      ...
      skip_spaces();
      bool is_sep_or_end =
          at_end() || peek() == ';' || (peek() == ',' && peek(1) == ' ') || (peek() == '|' && peek(1) == '|');
      restore(sp2);
      if (is_sep_or_end)
      {
        restore(sp);
    ...
    {
      comment_only = !mod->kind && mod->comment.has_value();
      kind = mod->kind.value_or(RuleKind::Open);
      comment = std::move(mod->comment);
    }

    // A comment-only modifier means "unknown, see the comment" — the old
    // OM stack and opening_hours.js agree; a definite state needs an
    // explicit open/closed keyword.
    if (comment_only)
    {
      kind = RuleKind::Unknown;
      // A rule that is nothing but a comment ('...; "on appointment"',
      // '..., "on request"') must not shadow or blanket the preceding
      // rules however it is attached: evaluate it as a fallback.
      if (day_sel.is_empty() && time_sel.is_00_24())
        op = RuleOperator::Fallback;
    }
    ...
    if (extra_comment)
      comments.push_back(std::move(*extra_comment));

    return RuleSequence{std::move(day_sel), std::move(time_sel), kind, op, std::move(comments)};
  }

  // -- Rules modifier --
  struct RulesModifier
  {
    std::optional<RuleKind> kind;  // nullopt: just a comment, no explicit keyword
    std::optional<std::string> comment;
  };

  std::optional<RulesModifier> try_parse_rules_modifier()
  {
    // Try comment alone
    auto sp = save();
    if (auto c = try_parse_comment())
      return RulesModifier{std::nullopt, std::move(*c)};
    restore(sp);
    ...
    return RulesModifier{*kind, std::move(c)};
  }
  ...
  std::optional<RuleKind> try_parse_rule_kind()
  {
    ...
    if (match_str("off"))
      return RuleKind::Closed;
    if (match_str("open"))
      return RuleKind::Open;
    // Widespread OSM-ism ("PH on").
    if (match_str("on"))
      return RuleKind::Open;
  ...
  std::optional<SelectorResult> parse_selector_sequence()
  {
    ...
    if (wide && wide->comment && wide->year.empty() && wide->monthday.empty() && wide->week.empty())
      match_str("24/7");
  ...
  std::optional<WideResult> parse_wide_range_selectors()
  {
    ...
    // Alt 2: monthday_selector week_selector? separator_for_readability?
    {
      ...
      if (auto md = try_parse_monthday_selector())
      {
        ...
        if (auto wk = try_parse_week_selector())
          wr.week = std::move(*wk);
    ...
    {
      // Single years favor the monthday prefix via Alt 2 ("2019 Apr 10 -
      // 2051 May 19"), so only ranges, lists and open ends land here; a space
      // may separate them from a month ("2020-2022 Jan 10:00-18:00").
      if (auto yr = try_parse_year_selector())
      {
        wr.year = std::move(*yr);
        skip_spaces();
      }
  ...
  std::optional<SmallResult> try_parse_small_range_selectors()
  {
    ...
    // Alt 1: weekday_selector space time_selector
    {
      ...
      if (auto wd = try_parse_weekday_selector())
      {
        ...
        if (match_space())
        {
          // Widespread tolerance: "Sa-Su 24/7" means open all day on those
          // days (the highest-weight unhandled family in the corpus).
          if (match_str("24/7"))
          {
            sr.weekday = std::move(*wd);
            sr.time.push_back(TimeSpan::fixed_range(ExtendedTime::midnight_00(), ExtendedTime::midnight_24()));
            return sr;
          }
    ...
    // Alt 3: time_selector alone. The same "24/7" tolerance covers wide-range
    // prefixes: "week 20-37 24/7", "May-Oct: 24/7".
    if (match_str("24/7"))
    {
      sr.time.push_back(TimeSpan::fixed_range(ExtendedTime::midnight_00(), ExtendedTime::midnight_24()));
      return sr;
  ...
  {
    if (auto hm = try_parse_hour_minutes())
      return Time::make_fixed(*hm);
    if (auto vt = try_parse_variable_time())
      return Time::make_variable(*vt);
    return std::nullopt;
  }

  std::optional<Time> try_parse_extended_time()
  {
    if (auto hm = try_parse_extended_hour_minutes())
      return Time::make_fixed(*hm);
    if (auto vt = try_parse_variable_time())
      return Time::make_variable(*vt);
    return std::nullopt;
  ...
  {
    if (match_str("dawn"))
      return TimeEvent::Dawn;
    if (match_str("sunrise"))
      return TimeEvent::Sunrise;
    if (match_str("sunset"))
      return TimeEvent::Sunset;
    if (match_str("dusk"))
      return TimeEvent::Dusk;
    return std::nullopt;
  }
  ...
  std::optional<ExtendedTime> try_parse_hour_minutes()
  {
    ...
    // Try "24:00" first
    if (match_str("24:00"))
      return ExtendedTime::midnight_24();
  ...
  std::optional<std::vector<WeekDayRange>> try_parse_weekday_selector()
  {
    // Try holiday_sequence first, then optional weekday_sequence
    {
      ...
      if (auto hol = try_parse_holiday_sequence())
      {
        ...
        auto sp2 = save();
        // Allow a spaced list ("PH, Sa,Su"), like weekday sequences do.
        if ((match_comma_spaces() || match_space()) && !at_end())
        {
    ...
    // Try weekday_sequence, then optional holiday_sequence
    {
      ...
      if (auto wd = try_parse_weekday_sequence())
      {
        ...
        auto sp2 = save();
        // A comma followed by whitespace can continue a weekday selector with
        // a holiday selector ("Tu-Su, PH 10:00-18:00").
        if ((match_comma_spaces() || match_space()) && !at_end())
        {
          if (auto hol = try_parse_holiday_sequence())
          {
  ...
  std::optional<std::vector<WeekDayRange>> try_parse_weekday_sequence()
  {
    ...
    {
      auto sp = save();
      if (match_comma_spaces())  // allow "Su, Sa" list, not just "Su,Sa"
      {
    ...
    if (peek() == '[')
    {
      ...
      auto parse_one_nth_entry = [&]() -> bool
      {
        ...
        if (match_char('-'))
        {
          ...
          {
            end_n = *n2;
            // Reject inverted ranges like inverted year/week ranges: the
            // empty nth set would never match yet print as a plain weekday.
            if (end_n < start_n)
              set_fatal("inverted nth range " + std::to_string(start_n) + "-" + std::to_string(end_n));
          }
          else
          {
            restore(sp3);
        ...
        for (uint8_t k = start_n; k <= end_n; ++k)
          if (negative)
            wr.nth_from_end[k - 1] = true;
          else
            wr.nth_from_start[k - 1] = true;
        return true;
      };

      if (!parse_one_nth_entry())
      {
        restore(sp);
        return std::nullopt;
      }
  ...
  std::optional<std::vector<WeekDayRange>> try_parse_holiday_sequence()
  {
    ...
    {
      auto sp = save();
      if (match_comma_spaces())  // allow "PH, SH" list, not just "PH,SH"
      {
  ...
  std::optional<WeekDayRange> try_parse_holiday()
  {
    HolidayKind hk;
    if (match_str("PH"))
      hk = HolidayKind::Public;
    else if (match_str("SH"))
      hk = HolidayKind::School;
    else
      return std::nullopt;

    int64_t off = 0;
    if (auto o = try_parse_day_offset())
      off = *o;
  ...
  std::optional<int64_t> try_parse_day_offset()
  {
    ...
    // Offsets beyond a year are nonsense, and huge numbers would silently
    // wrap through the int64 cast and chrono::days' int representation
    // ("Sa +4294967296 days" behaved exactly like "Sa").
    if (*num > 366)
    {
      restore(sp);
      return std::nullopt;
  ...
  std::optional<std::vector<WeekRange>> try_parse_week_selector()
  {
    ...
    {
      auto sp2 = save();
      if (match_comma_spaces())  // allow "week 1, 3" list
      {
    ...
    if (match_char('-'))
    {
      if (auto end = try_parse_weeknum())
      {
        ...
        if (match_char('/'))
        {
          ...
          {
            // Clamp before narrowing: a step of 256 must not truncate to 0
            // (division by zero in evaluation). Oversized steps clamp to 1,
            // matching Rust's `WeekRange::new`.
            wr.step = *step > static_cast<uint64_t>(wr.end - wr.start) ? 1 : static_cast<uint8_t>(*step);
          }
          else
          {
            restore(sp2);
  ...
  std::optional<std::vector<MonthdayRange>> try_parse_monthday_selector()
  {
    ...
    {
      auto sp = save();
      if (match_comma_spaces())  // allow "Jan, Mar" list, not just "Jan,Mar"
      {
  ...
  std::optional<MonthdayRange> finish_monthday_range_from_date(Date start_date)
  {
    ...
    if (auto off = try_parse_date_offset())
      start_offset = *off;
    ...
    // Check for "+"
    {
      ...
      if (match_char('+'))
      {
        Date end_date = start_date.has_year() ? Date::make_fixed(9999, Month::Dec, 31)
  ...
  std::optional<Date> try_parse_date_to(Date const & from)
  {
    // Try full date_from
    {
      auto sp = save();
  ...
  std::optional<DateOffset> try_parse_date_offset()
  {
    ...
    {
      auto sp2 = save();
      // Accept the optional space emitted before weekday date offsets.
      match_char(' ');
      auto sign = try_parse_plus_or_minus();
      if (sign)
      {
  ...
  std::optional<uint8_t> try_parse_daynum()
  {
    ...
    // unless that minute is also followed by ":" minute
    {
      ...
      {
        advance();
        if (std::isdigit(static_cast<unsigned char>(peek())) && std::isdigit(static_cast<unsigned char>(peek(1))))
        {
          // Check if followed by another ":MM"
          size_t after_min = pos_ + 2;
          if (after_min < input_.size() && input_[after_min] == ':' && after_min + 1 < input_.size() &&
  ...
  std::optional<std::vector<YearRange>> try_parse_year_selector()
  {
    ...
    {
      auto sp = save();
      if (match_comma_spaces())  // allow "2020, 2022" list
      {
  ...
  std::optional<YearRange> try_parse_year_range()
  {
    ...
    if (match_char('-'))
    {
      if (auto end = try_parse_year())
      {
        ...
        if (yr.end < yr.start)
          set_fatal("inverted year range " + std::to_string(yr.start) + "-" + std::to_string(yr.end));
        auto sp2 = save();
        if (match_char('/'))
        {
          if (auto step = try_parse_positive_number())
          {
            // Clamp before narrowing: a step of 65536 must not truncate to 0
            // (division by zero in evaluation). Oversized steps clamp to 1,
            // matching Rust's `YearRange::new`.
            yr.step = *step > static_cast<uint64_t>(yr.end - yr.start) ? 1 : static_cast<uint16_t>(*step);
          }
          else
          {
            restore(sp2);
  ...
  std::optional<uint64_t> try_parse_positive_number()
  {
    ...
    if (n == 0)
      return std::nullopt;  // must be positive
    return n;
  }
...
// ---------------------------------------------------------------------------
inline std::expected<OpeningHoursExpression, std::string> parse_normalized(std::string_view input)
{
...
inline void merge_timespans(std::vector<TimeSpan> & spans)
{
  ...
  // Sort the Fixed-start spans among themselves; sun-event spans keep their
  // positions. (A single comparator over the mixed set that "skips" variable
  // starts is not a strict weak ordering, which std::stable_sort requires.)
  {
    std::vector<size_t> idx;
    for (size_t i = 0; i < spans.size(); ++i)
      if (spans[i].start.tag == Time::Fixed)
        idx.push_back(i);
    std::vector<TimeSpan> fixed;
    fixed.reserve(idx.size());
    for (size_t i : idx)
      fixed.push_back(spans[i]);
    std::stable_sort(fixed.begin(), fixed.end(),
                     [](TimeSpan const & a, TimeSpan const & b) { return a.start.fixed < b.start.fixed; });
    for (size_t k = 0; k < idx.size(); ++k)
      spans[idx[k]] = std::move(fixed[k]);
  }
  ...
  for (size_t i = 1; i < spans.size(); ++i)
  {
    ...
    // Only strictly increasing fixed spans are linear. Wrapping and
    // equal-ended spans cross into the next day, so raw clock comparison would
    // discard their after-midnight interval.
    if (prev.start.tag == Time::Fixed && prev.end.tag == Time::Fixed && cur.start.tag == Time::Fixed &&
        cur.end.tag == Time::Fixed && !prev.open_end && !cur.open_end && !prev.repeats_minutes &&
        !cur.repeats_minutes && prev.start.fixed < prev.end.fixed && cur.start.fixed < cur.end.fixed)
    {
...
{
  std::stable_sort(v.begin(), v.end(), [](YearRange const & a, YearRange const & b) { return a.start < b.start; });
  dedup_vec(v);
}
...
{
  std::stable_sort(v.begin(), v.end(), [](WeekRange const & a, WeekRange const & b) { return a.start < b.start; });
  dedup_vec(v);
}
...
{
  std::stable_sort(v.begin(), v.end(), [](WeekDayRange const & a, WeekDayRange const & b)
  {
    if (a.kind != b.kind)
...
inline OpeningHoursExpression dedup(OpeningHoursExpression expr)
{
  ...
  // Remove fully duplicate rule sequences. Under the strict ';' override
  // semantics the last occurrence is authoritative ("Mo 10:00-12:00;
  // Mo 14:00-16:00; Mo 10:00-12:00" reopens the morning), so drop the
  // earlier copy -- unless a fallback rule sits in between, as fallback
  // grouping depends on position.
  {
    std::vector<RuleSequence> unique_rules;
    for (size_t i = 0; i < expr.rules.size(); ++i)
    {
      bool duplicatedLater = false;
      for (size_t j = i + 1; j < expr.rules.size() && !duplicatedLater; ++j)
      {
        if (expr.rules[j].op == RuleOperator::Fallback)
          break;
        duplicatedLater = expr.rules[i] == expr.rules[j];
      }
      if (!duplicatedLater)
        unique_rules.push_back(std::move(expr.rules[i]));
    }
...
inline std::expected<OpeningHoursExpression, std::string> parse(std::string_view input)
{
