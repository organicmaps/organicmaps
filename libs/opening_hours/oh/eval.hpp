// eval.hpp – C++23 evaluation engine for opening_hours
// ============================================================================
//
// This is the C++ port of the Rust `opening-hours` evaluation crate (the
// `opening-hours-syntax` crate is ported in oh/parser.hpp, which this header
// includes). It turns a parsed `OpeningHoursExpression` plus a `Context`
// into:
//
...
//
// Everything lives in `namespace oh`, depends only on the C++23
...
//
// Tested by opening_hours_tests/oh_eval_tests.cpp (vectors transcribed from
// the Rust suite).
//
// ============================================================================
#pragma once
...
struct NaiveDate
{
  ...
  auto operator<=>(NaiveDate const &) const = default;
  bool operator==(NaiveDate const &) const = default;
};
...
struct IsoWeek
{
  int year;
  int week;
  auto operator<=>(IsoWeek const &) const = default;
};
...
inline std::optional<NaiveDate> from_isoywd(int iso_year, int week, Weekday wd)
{
  ...
  if (check.year != iso_year || check.week != week)
    return std::nullopt;
  return result;
}
...
inline std::optional<NaiveDate> from_weekday_of_month(int y, unsigned m, Weekday wd, int nth)
{
  ...
  if (unsigned(back.month()) != m || int(back.year()) != y)
    return std::nullopt;
  return NaiveDate{sd};
}
...
inline NaiveDate apply_date_offset(DateOffset const & off, NaiveDate date)
{
...
struct NaiveDateTime
{
  ...
  NaiveDateTime(NaiveDate d, int minute_of_day) { *this = normalized(d.d, minute_of_day); }
  NaiveDateTime(chrono::sys_days d, int minute_of_day) { *this = normalized(d, minute_of_day); }
  ...
  NaiveDateTime add_minutes(int64_t m) const { return normalized(date_, int64_t(minute_) + m); }

  auto operator<=>(NaiveDateTime const &) const = default;
  bool operator==(NaiveDateTime const &) const = default;
};

// --- Global evaluation bounds (opening_hours.rs:22-33) ---

inline NaiveDate const DATE_START_DATE = NaiveDate::ymd_unchecked(1900, 1, 1);
inline NaiveDate const DATE_END_DATE = NaiveDate::ymd_unchecked(10000, 1, 1);
inline NaiveDateTime const DATE_START{DATE_START_DATE, 0};
inline NaiveDateTime const DATE_END{DATE_END_DATE, 0};
...
struct CompactYear
{
  ...
  std::optional<std::pair<unsigned, unsigned>> first() const
  {
    for (unsigned i = 0; i < 12; ++i)
      if (auto d = months[i].first())
        return std::pair{i + 1, *d};
    return std::nullopt;
  }
  std::optional<std::pair<unsigned, unsigned>> first_after(unsigned month, unsigned day) const
  {
    if (auto d = months[month - 1].first_after(day))
      return std::pair{month, *d};
...
struct CompactCalendar
{
  ...
  CompactYear * year_for_mut(NaiveDate date)
  {
    ...
    if (idx < 0 || idx >= int64_t(calendar.size()))
      return nullptr;
    return &calendar[size_t(idx)];
  }
  CompactYear const * year_for(NaiveDate date) const
  {
    int64_t idx = int64_t(date.year()) - first_year;
    if (idx < 0 || idx >= int64_t(calendar.size()))
      return nullptr;
  ...
  bool insert(NaiveDate date)
  {
    ...
    {
      for (int i = date.year(); i < first_year; ++i)
        calendar.emplace_front();
      first_year = date.year();
      y = &calendar.front();
    }
    else
    {
      ...
      for (int i = last_year; i < date.year(); ++i)
        calendar.emplace_back();
      y = &calendar.back();
    }
  ...
  bool contains(NaiveDate date) const
  {
    if (auto * y = year_for(date))
      return y->contains(date.month(), date.day());
    return false;
  }
  ...
  {
    unsigned c = 0;
    for (auto const & y : calendar)
      c += y.count();
    return c;
  }

  bool operator==(CompactCalendar const & o) const
  {
    ...
    if (first_year == o.first_year && calendar.size() == o.calendar.size())
    {
      for (size_t i = 0; i < calendar.size(); ++i)
        for (unsigned m = 0; m < 12; ++m)
          if (calendar[i].months[m].bits != o.calendar[i].months[m].bits)
            return false;
      return true;
    }
...
private:
  bool contains_all(CompactCalendar const & o) const
  {
...
struct ETRange
{
  ExtendedTime start;
  ExtendedTime end;
  auto operator<=>(ETRange const &) const = default;
};
...
{
  std::sort(ranges.begin(), ranges.end(), [](ETRange const & a, ETRange const & b) { return a.start < b.start; });
...
struct TimeRange
{
  ...
  bool state_eq(TimeRange const & o) const { return kind == o.kind && comment == o.comment; }
...
struct Schedule
{
  ...
  static Schedule from_ranges(std::vector<ETRange> const & ranges, RuleKind kind, std::string const & comment)
  {
    std::vector<TimeRange> in;
    in.reserve(ranges.size());
    for (auto const & r : ranges)
      if (r.start < r.end)
        in.push_back(TimeRange{r.start, r.end, kind, comment});

    if (in.size() > 1)
    {
      std::sort(in.begin(), in.end(), [](TimeRange const & a, TimeRange const & b) { return a.start < b.start; });
      size_t i_kept = 0;
  ...
  {
    for (auto const & rg : inner)
      if (!(rg.kind == RuleKind::Closed && rg.comment.empty()))
        return false;
    return true;
  }
  ...
  Schedule insert(TimeRange ins) const
  {
    ...
    for (size_t i = ai; i < after.size(); ++i)
      result.push_back(std::move(after[i]));
    return Schedule{std::move(result)};
  }

  /// Merge another schedule into this one (fold `insert` over its ranges).
  Schedule addition(Schedule const & other) const
  {
    Schedule acc = *this;
    for (size_t i = other.inner.size(); i-- > 0;)
      acc = acc.insert(other.inner[i]);
    return acc;
  }
  ...
  std::vector<TimeRange> into_ranges() const
  {
    ...
    size_t i = 0;
    auto const & in = inner;
    auto is_holes = [](TimeRange const & tr) { return tr.kind == RuleKind::Closed && tr.comment.empty(); };
    ...
    while (last_end < ExtendedTime::midnight_24())
    {
      ...
      while (i < in.size())
      {
        TimeRange const & next_range = in[i];
      ...
      if (!stopped && is_holes(yielded))
        yielded.end = ExtendedTime::midnight_24();
  ...
  bool operator==(Schedule const &) const = default;
};
...
public:
  ...
  explicit ScheduleIter(Schedule const & s) : items_(s.into_ranges()) {}

  TimeRange const * peek() const { return idx_ < items_.size() ? &items_[idx_] : nullptr; }
...
{
  return std::fmod(m + eoc + std::fmod(argument_of_perihelion(day), 2.0 * kPi) + 3.0 * kPi, 2.0 * kPi);
}
...
inline double declination(double lambda)
{
  return std::asin(std::sin(lambda) * 0.39779);
}
...
inline double event_angle(TimeEvent event)
{
  ...
  case TimeEvent::Sunset: return deg2rad(5.0) / 6.0;  // ~0.8333°
  case TimeEvent::Dawn:
  case TimeEvent::Dusk: return deg2rad(6.0);  // civil
  }
  return 0.0;
}
...
struct Coordinates
{
  ...
  {
    if (std::isnan(lat) || std::isnan(lon) || lat < -90.0 || lat > 90.0 || lon < -180.0 || lon > 180.0)
      return std::nullopt;
...
inline ExtendedTime fixed_event_fallback(TimeEvent event)
{
  switch (event)
  {
  case TimeEvent::Dawn: return ExtendedTime{6, 0};
  case TimeEvent::Sunrise: return ExtendedTime{7, 0};
  case TimeEvent::Sunset: return ExtendedTime{19, 0};
  case TimeEvent::Dusk: return ExtendedTime{20, 0};
  }
...
/// DST-correct.
template <class L>
concept Localize = requires(L const & l, typename L::DateTime dt, NaiveDateTime naive, NaiveDate date, TimeEvent ev) {
  typename L::DateTime;
...
struct NoLocation
{
  ...
  auto operator<=>(NoLocation const &) const = default;
  bool operator==(NoLocation const &) const = default;
};
...
struct LocalResolution
{
  ...
  static LocalResolution ambiguous(int64_t earliest, int64_t latest) { return {Kind::Ambiguous, earliest, latest}; }
};
...
struct ZonedDateTime
{
  ...
  auto operator<=>(ZonedDateTime const &) const = default;
  bool operator==(ZonedDateTime const &) const = default;
};
...
template <class Tz>
concept TzProvider = requires(Tz const & tz, int64_t utc, NaiveDateTime local) {
...
struct TzLocation
{
  ...
  std::optional<Coordinates> const & get_coords() const { return coords; }
  Tz const & get_timezone() const { return tz; }
  ...
  ZonedDateTime datetime(NaiveDateTime naive) const
  {
    while (true)
    {
      ...
      if (r.kind != LocalResolution::Kind::None)
  ...
  bool operator==(TzLocation const &) const = default;
};
...
{
  std::shared_ptr<CompactCalendar const> public_;
  std::shared_ptr<CompactCalendar const> school;

  ContextHolidays() = default;
  ContextHolidays(std::shared_ptr<CompactCalendar const> pub, std::shared_ptr<CompactCalendar const> sch)
    : public_(std::move(pub))
    , school(std::move(sch))
  {}

  CompactCalendar const & get_for_kind(HolidayKind kind) const
  {
    static CompactCalendar const kEmpty;
    auto const & p = (kind == HolidayKind::Public) ? public_ : school;
    return p ? *p : kEmpty;
  }
};
...
inline bool time_selector_is_immutable_full_day(TimeSelector const & ts)
{
...
std::optional<ETRange> timespan_as_naive(TimeSpan const & span, Context<L> const & ctx, NaiveDate date)
{
  ...
  {
    if (span.end.tag != Time::Variable)
      return std::nullopt;
    bool is_morning = span.end.variable.event == TimeEvent::Sunrise || span.end.variable.event == TimeEvent::Dawn;
    if (is_morning == is_summer)
      return std::nullopt;
    else
...
{
  // Treat day 0 as day 1: it must not reach the descending clamp below,
  // where the unsigned wrap-around would spin through ~4 billion iterations.
  if (day == 0)
    day = 1;
  if (auto d = NaiveDate::from_ymd(year, month, day))
    return *d;
  for (unsigned dd = day - 1; dd >= 28; --dd)
    if (auto d = NaiveDate::from_ymd(year, month, dd))
      return *d;
...
inline std::vector<DateInterval> intervals_from_bounds(std::vector<NaiveDate> starts_in, std::vector<NaiveDate> ends_in)
{
  ...
  while (true)
  {
    if (si < starts.size())
    {
      ...
      while (ei < ends.size() && ends[ei] < start)
        ++ei;
    }
...
inline bool is_open_from_intervals(NaiveDate date, std::vector<DateInterval> const & intervals)
{
  for (auto const & rg : intervals)
    if (rg.last >= date)
      return rg.contains(date);
  return false;
}

inline NaiveDate next_change_from_intervals(NaiveDate date, std::vector<DateInterval> const & intervals)
{
  for (auto const & rg : intervals)
  {
    if (rg.last >= date)
...
{
  if (nth >= 0)
    return from_weekday_of_month(year, unsigned(month), wday, nth);
...
inline std::optional<NaiveDate> date_on_year(Date const & date, int for_year, DateBuilder builder)
{
  switch (date.kind)
  {
  case DateKind::Easter: return easter(date.year.has_value() ? int(*date.year) : for_year);
  case DateKind::Fixed:
    if (date.year.has_value() && int(*date.year) != for_year)
      return std::nullopt;
...
inline std::vector<DateInterval> monthday_range_to_intervals(NaiveDate date, MonthdayRange const & mr)
{
  std::vector<DateInterval> out;
  ...
  if (mr.kind == MonthdayRangeKind::MonthOnly)
  {
    ...
    {
      // Month range with an explicit start year. The range runs through
      // the LAST day of the end month ("2026 Apr-May" covers all of
      // May), mirroring the no-year branch above; the Rust reference
      // stops at the 1st of the end month (upstream bug).
      int year = int(*mr.month_year);
      for (int end_year : {year, year + 1})
      {
        auto start = NaiveDate::from_ymd(year, unsigned(mr.month_start), 1);
        int const wrap = mr.month_end == Month::Dec ? 1 : 0;
        auto end = NaiveDate::from_ymd(end_year + wrap, unsigned(next_month(mr.month_end)), 1);
        if (start && end && start->d < end->d)
        {
          out.push_back({*start, end->pred()});
          break;
        }
      }
      return out;
  ...
  {
    // +-8 years, not the Rust reference's +-4 (an OM deviation): around a
    // skipped century leap year the next Feb 29 is up to 8 years away
    // (2096 -> 2104), and +-4 would report "never opens" there.
    for (int y = year - 8; y <= year + 8; ++y)
    {
...
inline bool year_filter(YearRange const & yr, NaiveDate date)
{
  int year = date.year();
...
inline std::optional<NaiveDate> year_next_change_hint(YearRange const & yr, NaiveDate date)
{
  int curr = date.year();
  int start = yr.start, end = yr.end, step = yr.step;
  ...
  if (auto d = NaiveDate::from_ymd(next_year, 1, 1))
    return *d;
  return DATE_END_DATE;
}
...
inline bool monthday_filter(MonthdayRange const & mr, NaiveDate date)
{
  return is_open_from_intervals(date, monthday_range_to_intervals(date, mr));
}

inline std::optional<NaiveDate> monthday_next_change_hint(MonthdayRange const & mr, NaiveDate date)
{
...
inline bool week_filter(WeekRange const & wr, NaiveDate date)
{
...
inline std::optional<NaiveDate> week_next_change_hint(WeekRange const & wr, NaiveDate date)
{
...
inline bool weekday_fixed_filter_nowrap(Weekday rs, Weekday re, int64_t offset, bool const nfs[5], bool const nfe[5],
                                        NaiveDate date)
{
...
template <class L>
  requires Localize<L>
bool weekday_filter(Context<L> const & ctx, WeekDayRange const & w, NaiveDate date)
{
  if (w.kind == WeekDayRangeKind::FixedDays)
  {
    if (int(w.range_start) > int(w.range_end))
    {
      return weekday_fixed_filter_nowrap(w.range_start, Weekday::Sun, w.offset, w.nth_from_start, w.nth_from_end,
                                         date) ||
             weekday_fixed_filter_nowrap(Weekday::Mon, w.range_end, w.offset, w.nth_from_start, w.nth_from_end, date);
    }
    return weekday_fixed_filter_nowrap(w.range_start, w.range_end, w.offset, w.nth_from_start, w.nth_from_end, date);
  }
...
template <class L>
  requires Localize<L>
bool weekday_overrides_unknown(Context<L> const & ctx, WeekDayRange const & w, NaiveDate date)
{
  if (w.kind != WeekDayRangeKind::Holiday)
    return false;
  NaiveDate d = date.add_days(-w.offset);
...
template <class L>
  requires Localize<L>
std::optional<NaiveDate> weekday_next_change_hint(Context<L> const & ctx, WeekDayRange const & w, NaiveDate date)
{
  if (w.kind != WeekDayRangeKind::Holiday)
    return std::nullopt;

  NaiveDate date_with_offset = date.add_days(-w.offset);
  auto next_change_for = [&](CompactCalendar const & cal) -> NaiveDate
  {
...
bool vec_filter(Vec const & v, F && f)
{
  if (v.empty())
    return true;
  for (auto const & x : v)
...
template <class Vec, class F>
std::optional<NaiveDate> vec_hint(Vec const & v, F && f)
{
  if (v.empty())
    return DATE_END_DATE;
...
template <class L>
  requires Localize<L>
bool day_selector_filter(Context<L> const & ctx, DaySelector const & ds, NaiveDate date)
{
  return vec_filter(ds.year, [&](YearRange const & x) { return year_filter(x, date); }) &&
         vec_filter(ds.monthday, [&](MonthdayRange const & x) { return monthday_filter(x, date); }) &&
         vec_filter(ds.week, [&](WeekRange const & x) { return week_filter(x, date); }) &&
         vec_filter(ds.weekday, [&](WeekDayRange const & x) { return weekday_filter(ctx, x, date); });
}

template <class L>
  requires Localize<L>
bool day_selector_overrides_unknown(Context<L> const & ctx, DaySelector const & ds, NaiveDate date)
{
  // Only weekday-holiday selectors can force Unknown.
  return vec_any(ds.weekday, [&](WeekDayRange const & x) { return weekday_overrides_unknown(ctx, x, date); });
}

template <class L>
  requires Localize<L>
std::optional<NaiveDate> day_selector_next_change_hint(Context<L> const & ctx, DaySelector const & ds, NaiveDate date)
{
  if (ds.is_empty())
    return DATE_END_DATE;
  auto h = vec_hint(ds.year, [&](YearRange const & x) { return year_next_change_hint(x, date); });
  h = min_opt(h, vec_hint(ds.monthday, [&](MonthdayRange const & x) { return monthday_next_change_hint(x, date); }));
  h = min_opt(h, vec_hint(ds.week, [&](WeekRange const & x) { return week_next_change_hint(x, date); }));
  h = min_opt(h, vec_hint(ds.weekday, [&](WeekDayRange const & x) { return weekday_next_change_hint(ctx, x, date); }));
  return h;
}
...
inline std::string rule_comment(RuleSequence const & rs)
{
  std::string out;
...
inline std::pair<RuleKind, std::string> rule_state(RuleSequence const & rs)
{
  return {rs.kind, rule_comment(rs)};
}

/// `OpeningHoursExpression::is_constant()` (rules/mod.rs:33-48).
inline bool expr_is_constant(OpeningHoursExpression const & expr)
{
  if (expr.rules.empty())
    return true;
  ...
  RuleSequence const * tail = nullptr;
  for (auto it = expr.rules.rbegin(); it != expr.rules.rend(); ++it)
  {
    if (it->day_selector.is_empty() || !it->time_selector.is_00_24() || rule_state(*it) != state)
    {
      tail = &*it;
      break;
    }
...
{
  auto build = [&](NaiveDate d, std::vector<ETRange> const & intervals) -> std::optional<Schedule>
  {
    if (!day_selector_filter(ctx, rs.day_selector, d))
      return std::nullopt;
    RuleKind kind = day_selector_overrides_unknown(ctx, rs.day_selector, d) ? RuleKind::Unknown : rs.kind;
...
template <class L>
  requires Localize<L>
Schedule compute_schedule_at(OpeningHoursExpression const & expr, Context<L> const & ctx, NaiveDate date)
{
  if (!(DATE_START_DATE <= date && date < DATE_END_DATE))
    return Schedule{};

  bool prev_match = false;
  std::optional<Schedule> prev_eval;

  for (auto const & rs : expr.rules)
  {
    ...
    bool normal_open_unknown =
        rs.op == RuleOperator::Normal && (rs.kind == RuleKind::Open || rs.kind == RuleKind::Unknown);
    bool overlay = rs.op == RuleOperator::Additional || (rs.op == RuleOperator::Normal && rs.kind == RuleKind::Closed);
  ...
  if (prev_eval)
    return prev_eval->filter_closed_ranges();
  return Schedule{};
}
...
template <class L>
  requires Localize<L>
std::optional<NaiveDate> compute_next_change_hint(OpeningHoursExpression const & expr, Context<L> const & ctx,
                                                  NaiveDate date)
{
...
{
  std::shared_ptr<OpeningHoursExpression const> expr_;
  Context<L> ctx_;
  NaiveDate curr_date_;
  ScheduleIter curr_schedule_;
  NaiveDateTime end_datetime_;

  static bool range_contains(TimeRange const & tr, ExtendedTime t) { return tr.start <= t && t < tr.end; }

public:
  TimeDomainIterator(std::shared_ptr<OpeningHoursExpression const> expr, Context<L> ctx, NaiveDateTime start_datetime,
                     NaiveDateTime end_datetime)
    : expr_(std::move(expr))
    , ctx_(std::move(ctx))
    , curr_date_(start_datetime.date())
    , end_datetime_(end_datetime)
  {
    ExtendedTime start_time{uint8_t(start_datetime.hour()), uint8_t(start_datetime.minute_of_hour())};
  ...
  {
    TimeRange const * peek = curr_schedule_.peek();
    if (!peek)
      return std::nullopt;
    ...
    ExtendedTime end_time = curr_schedule_.peek() ? curr_schedule_.peek()->start : ExtendedTime::midnight_00();
...
private:
  void consume_until_next_state(std::pair<RuleKind, std::string> curr_state)
  {
    ...
    {
      TimeRange const * p = curr_schedule_.peek();
    ...
    while (peek_state_matches())
    {
      ...
      if (!curr_schedule_.peek())
      {
        ...
        if (!(next_hint > curr_date_))
          next_hint = curr_date_.succ();
        curr_date_ = next_hint;

        if (curr_date_ > end_datetime_.date() || DATE_END_DATE <= curr_date_)
          break;
...
template <class L>
  requires Localize<L>
std::optional<DateTimeRange<NaiveDateTime>> first_naive_interval(
    std::shared_ptr<OpeningHoursExpression const> const & expr, Context<L> const & ctx, NaiveDateTime from,
    NaiveDateTime to)
{
...
template <class L>
  requires Localize<L>
std::vector<DateTimeRange<NaiveDateTime>> collect_naive_range(
    std::shared_ptr<OpeningHoursExpression const> const & expr, Context<L> const & ctx, NaiveDateTime from,
    NaiveDateTime to)
{
  NaiveDateTime f = std::min(DATE_END, from);
...
public:
  ...
  explicit OpeningHours(OpeningHoursExpression expr, Context<L> ctx = {})
    : expr_(std::make_shared<OpeningHoursExpression const>(std::move(expr)))
    , ctx_(std::move(ctx))
  {}
  OpeningHours(std::shared_ptr<OpeningHoursExpression const> expr, Context<L> ctx)
    : expr_(std::move(expr))
    , ctx_(std::move(ctx))
  {}

  Context<L> const & get_context() const { return ctx_; }
  std::shared_ptr<OpeningHoursExpression const> get_expression() const { return expr_; }
  OpeningHoursExpression const & expression() const { return *expr_; }
  ...
  Schedule schedule_at(NaiveDate date) const { return compute_schedule_at(*expr_, ctx_, date); }
  ...
  std::pair<RuleKind, std::string> state(DateTime t) const
  {
    ...
    if (first)
      return {first->kind, first->comment};
  ...
  std::optional<DateTime> next_change(DateTime current_time) const
  {
    ...
    if (ctx_.locale.naive(end_dt) >= DATE_END)
      return std::nullopt;
    return end_dt;
  }
  ...
  std::vector<DateTimeRange<DateTime>> iter_range(DateTime from, DateTime to) const
  {
    ...
    for (auto & dtr : naive)
    {
      out.push_back(DateTimeRange<DateTime>{ctx_.locale.datetime(dtr.start), ctx_.locale.datetime(dtr.end), dtr.kind,
...
inline std::expected<OpeningHours<NoLocation>, std::string> parse_opening_hours(std::string_view raw)
{
  auto expr = parse(raw);
  if (!expr)
