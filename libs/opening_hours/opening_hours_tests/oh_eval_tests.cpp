// Tests for the C++23 opening hours evaluator (oh/eval.hpp).
//
...
namespace oh_eval_tests
{
using namespace oh;

// Collect the streamed upstream message and assert through the OM framework.
static char const * g_current = "";

#define OH_CHECK(cond, msg)                       \
  do                                              \
  {                                               \
    bool const passed_ = static_cast<bool>(cond); \
    if (!passed_)                                 \
    {                                             \
      std::ostringstream oss_;                    \
      oss_ << msg;                                \
      TEST(passed_, (g_current, oss_.str()));     \
    }                                             \
  }                                               \
  while (0)
...
static std::string render(Schedule const & s)
{
  std::string out;
  bool first = true;
  for (auto const & tr : s.into_ranges())
  {
    if (!first)
      out += " | ";
    first = false;
...
static void check_schedule(OH_T & oh, std::string_view date, std::string_view expected_dsl)
{
  ...
  OH_CHECK(got == want, "schedule_at(" << oh.to_string() << " @ " << date << ")\n      got:  " << render(got)
                                       << "\n      want: " << render(want));
}
...
{
  Schedule got = oh.schedule_at(pd(date));
  OH_CHECK(got.is_empty(),
           "schedule_at(" << oh.to_string() << " @ " << date << ") expected empty, got: " << render(got));
}
...
static void check_state(std::string_view expr, std::string_view dt, RuleKind expected)
{
  ...
  OH_CHECK(st.first == expected,
           "state(" << expr << " @ " << dt << ") = " << rule_kind_str(st.first) << " want " << rule_kind_str(expected));
}
...
static void check_next_change(std::string_view dt, std::string_view expr, std::string_view expected)
{
  ...
  if (!nc)
  {
    OH_CHECK(false, "next_change(" << expr << " @ " << dt << ") = <none>, want " << expected);
    return;
  }
  NaiveDateTime want = pdt(expected);
  OH_CHECK(*nc == want, "next_change(" << expr << " @ " << dt << ") mismatch, want " << expected);
}
...
static void check_no_next_change(std::string_view dt, std::string_view expr)
{
  auto oh = OH(expr);
  auto nc = oh.next_change(pdt(dt));
  OH_CHECK(!nc.has_value(), "next_change(" << expr << " @ " << dt << ") should be <none>");
}

// ===========================================================================
// eval_state.rs
// ===========================================================================
UNIT_TEST(eval_state)
{
...
UNIT_TEST(eval_schedule)
{
  ...
  // Time span
  {
    auto o = S("14:00-19:00");
    check_schedule(o, "2020-06-01", "14:00 open 19:00");
  }
  {
    auto o = S("Mo 14:00-19:00");
    check_schedule(o, "2020-06-01", "14:00 open 19:00");
  }
  {
    auto o = S("Su 14:00-19:00");
    check_schedule_empty(o, "2020-06-01");
  }
  {
    auto o = S("Su 14:00-25:30");
    check_schedule(o, "2020-06-01", "00:00 open 01:30");
  }
  {
    auto o = S("10:00-12:00,11:00-16:00 unknown");
    check_schedule(o, "2020-06-01", "10:00 unknown 16:00");
  }
  {
    auto o = S("23:00-01:00");
    check_schedule(o, "2020-06-01", "00:00 open 01:00 | 23:00 open 24:00");
  }
  {
    auto o = S("Mo 04:00-04:00");
    check_schedule_empty(o, "2025-02-23");
  }
  {
    auto o = S("Mo 04:00-04:00");
    check_schedule(o, "2025-02-24", "04:00 open 24:00");
  }
  {
    auto o = S("Mo 04:00-04:00");
    check_schedule(o, "2025-02-25", "00:00 open 04:00");
  }
  {
    auto o = S("Mo 04:00-04:00");
    check_schedule_empty(o, "2025-02-26");
  }
  {
    auto o = S("Mo 10:00-17:30,12:30-12:30");
    check_schedule(o, "2025-02-24", "10:00 open 24:00");
    check_schedule(o, "2025-02-25", "00:00 open 12:30");
  }
  {
    auto o = S("10:00-12:00,14:00-16:00");
    check_schedule(o, "2020-06-01", "10:00 open 12:00 | 14:00 open 16:00");
  }
  {
    auto o = S("10:00-12:00,14:00-25:30");
    check_schedule(o, "2020-06-01", "00:00 open 01:30 | 10:00 open 12:00 | 14:00 open 24:00");
  }
  // Weekday range
  {
    auto o = S("Mo-Su");
  ...
  {
    auto o = S("Tu");
    check_schedule(o, "2020-06-02", "00:00 open 24:00");
  }
  {
    auto o = S("We");
    check_schedule_empty(o, "2020-06-02");
  }
  {
  ...
  // Weekday nth
  {
    auto o = S("Mo[1] 10:00-12:00");
    check_schedule(o, "2020-06-01", "10:00 open 12:00");
    check_schedule_empty(o, "2020-06-08");
  }
  {
  ...
  // Week range
  {
    auto o = S("week01:10:00-12:00");
    check_schedule(o, "2020-01-01", "10:00 open 12:00");
    check_schedule_empty(o, "2020-01-06");
  }
  {
    auto o = S("week01,22-23:10:00-12:00");
    check_schedule(o, "2020-05-31", "10:00 open 12:00");
    check_schedule(o, "2020-06-07", "10:00 open 12:00");
  }
  {
  ...
  // Month selector
  {
    auto o = S("2020Jun01:10:00-12:10");
    check_schedule(o, "2020-06-01", "10:00 open 12:10");
  }
  {
    auto o = S("Jan-Jun:11:58-11:59");
    check_schedule(o, "2020-06-01", "11:58 open 11:59");
  }
  {
    auto o = S("May15-01:10:00-12:00");
    check_schedule(o, "2020-06-01", "10:00 open 12:00");
    check_schedule_empty(o, "2020-06-02");
  }
  {
    auto o = S("2019Sep01-2020Jul31:10:00-12:00");
    check_schedule(o, "2020-06-01", "10:00 open 12:00");
  }
  {
    auto o = S("Sep01-Jul01:10:00-12:00");
    check_schedule(o, "2020-06-01", "10:00 open 12:00");
  }
  // Month OOB
  {
  ...
  // Year selector
  {
    auto o = S("2020:10:00-12:00");
    check_schedule(o, "2020-01-01", "10:00 open 12:00");
    check_schedule_empty(o, "2021-01-01");
  }
  {
  ...
  // Rule normal
  {
    auto o = S("Jun ; 00:00-04:00 closed");
    check_schedule(o, "2020-06-01", "04:00 open 24:00");
    check_schedule_empty(o, "2020-07-01");
  }
  {
    auto o = S("Sa,Su 11:00-13:45 open; 10:00-18:00");
    check_schedule(o, "2020-06-01", "10:00 open 18:00");
    check_schedule(o, "2020-05-31", "10:00 open 18:00");
  }
  // Rule additional
  {
    auto o = S("10:00-12:00 open, 14:00-16:00 unknown, 16:00-23:00 closed");
    check_schedule(o, "2020-06-01", "10:00 open 12:00 | 14:00 unknown 16:00");
  }
  {
    auto o = S("10:00-20:00 open, 12:00-14:00 closed");
    check_schedule(o, "2020-06-01", "10:00 open 12:00 | 14:00 open 20:00");
  }
  {
    auto o = S("12:00-14:00 closed, 10:00-20:00 open");
    check_schedule(o, "2020-06-01", "10:00 open 20:00");
  }
  // Rule fallback
  {
    auto o = S("Jun:10:00-12:00 open || unknown");
    check_schedule(o, "2020-06-01", "10:00 open 12:00");
    check_schedule(o, "2020-05-31", "00:00 unknown 24:00");
  }
  {
  ...
  // Comments
  {
    auto o = S(R"(10:00-12:00 open "welcome!")");
    check_schedule(o, "2020-06-01", "10:00 open[welcome!] 12:00");
  }
  // A comment-only modifier means "unknown, see the comment", matching the
  // app-facing opening-hours contract and opening_hours.js.
  {
    auto o = S(R"(10:00-18:00 "may close later" ; 12:00-13:00 closed "ring the bell")");
    check_schedule(o, "2024-01-01",
                   "10:00 unknown[may close later] 12:00 closed[ring the bell] 13:00 unknown[may close later] 18:00");
  }
  {
    auto o = S(R"(10:00-18:00 "may close later", 12:00-13:00 "ring the bell")");
    check_schedule(o, "2024-01-01",
                   "10:00 unknown[may close later] 12:00 unknown[ring the bell] 13:00 unknown[may close later] 18:00");
  }
  {
    auto o = S(R"(10:00-14:00 "may open earlier", 14:00-18:00 "may close later")");
    check_schedule(o, "2024-01-01", "10:00 unknown[may open earlier] 14:00 unknown[may close later] 18:00");
  }
  // Time events (fixed fallback, NoLocation): dawn 06:00 sunrise 07:00 sunset 19:00 dusk 20:00
  {
    auto o = S("(dawn-02:30)-(dusk+02:30)");
    check_schedule(o, "2020-06-01", "03:30 open 22:30");
  }
  {
    auto o = S("(dawn+00:30)-(dusk-00:30)");
    check_schedule(o, "2020-06-01", "06:30 open 19:30");
  }
  {
    auto o = S("sunrise-19:45");
    check_schedule(o, "2020-06-01", "07:00 open 19:45");
  }
  {
    auto o = S("08:15-sunset");
    check_schedule(o, "2020-06-01", "08:15 open 19:00");
  }
}
...
struct FixedOffset
{
  ...
  bool operator==(FixedOffset const &) const = default;
};
...
struct EuroParis
{
  ...
  {
    NaiveDate d = NaiveDate::ymd_unchecked(year, month, count_days_in_month(NaiveDate::ymd_unchecked(year, month, 1)));
  ...
  LocalResolution from_local(NaiveDateTime local) const
  {
    ...
    if (v1 && v2)
      return LocalResolution::ambiguous(std::min(u1, u2), std::max(u1, u2));
    if (v1)
      return LocalResolution::single(u1);
    if (v2)
      return LocalResolution::single(u2);
    return LocalResolution::none();
  }
  bool operator==(EuroParis const &) const = default;
};

template <class Tz>
static ZonedDateTime zdt(Tz const & tz, std::string_view s)
{
...
UNIT_TEST(schedule_with_tz)
{
  ...
  {
    auto o = P("sunrise-19:45");
    check_schedule(o, "2020-06-01", "05:51 open 19:45");
  }
  {
    auto o = P("08:15-sunset");
    check_schedule(o, "2020-06-01", "08:15 open 21:46");
  }
  {
    auto o = P("(dawn+00:30)-(dusk-00:30)");
    check_schedule(o, "2020-06-01", "05:40 open 21:57");
  }
  {
    auto o = P("(dawn-02:30)-(dusk+02:30)");
    check_schedule(o, "2020-06-01", "00:00 open 00:56 | 02:40 open 24:00");
  }

  // At the poles, sun events may not happen (seasonal fallback).
  {
    auto o = A("sunrise-20:00");
    check_schedule(o, "2020-04-01", "09:11 open 20:00");
  }
  {
    auto o = A("10:00-sunset");
    check_schedule(o, "2020-04-01", "10:00 open 18:15");
  }
  {
    auto o = A("sunset-dusk");
    check_schedule(o, "2020-04-01", "18:15 open 20:16");
  }
  {
...
UNIT_TEST(localization_tz)
{
  ...
  // Plain tz: next change lands on a normal boundary.
  {
    ...
    OH_CHECK(nc.has_value() && *nc == zdt(tz, "2024-12-23 18:00"), "ctx_with_tz");
  }
  // Spring-forward: 02:30 does not exist -> snaps to 03:00.
  {
    ...
    OH_CHECK(nc.has_value() && *nc == zdt(tz, "2024-03-31 03:00"), "ends_at_invalid_time");
  }
  // Fall-back: end at 02:30 of the following (CET) day.
  {
    auto oh = OH("10:00-26:30").with_context(ctx);
    auto nc = oh.next_change(zdt(tz, "2024-10-27 14:44"));
    OH_CHECK(nc.has_value() && *nc == zdt(tz, "2024-10-28 02:30"), "ends_at_ambiguous_time");
  }
  ...
  // Sun events with inferred-tz-equivalent coordinates.
  {
    ...
    auto nc = oh.next_change(zdt(tz, "2024-12-23 14:44"));
    OH_CHECK(nc.has_value() && *nc == zdt(tz, "2024-12-23 16:57"), "infer_tz sunset");
    ...
    OH_CHECK(nc2.has_value() && *nc2 == zdt(tz, "2024-07-15 06:03"), "infer_all sunrise after PH");
  }
}
