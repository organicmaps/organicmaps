...
UNIT_TEST(CzechPragueHiltonToKarlovMost)
{
  integration::CalculateRouteAndTestRouteLength(integration::GetVehicleComponents(VehicleType::Pedestrian),
                                                ...
                                                mercator::FromLatLon(50.0864, 14.4124), 2427);
}
...
UNIT_TEST(Russia_Moscow_HydroprojectBridgeCrossing_TurnTest)
{
  ...
  integration::TestRouteLength(route, 261.349);
  ...
  TEST_EQUAL(t[3].m_pedestrianTurn, PedestrianDirection::TurnLeft, ());
  TEST_EQUAL(t[4].m_pedestrianTurn, PedestrianDirection::TurnLeft, ());
...
UNIT_TEST(Belarus_Minsk_RenaissanceHotelUndergroundCross_TurnTest)
{
  ...
  integration::TestRouteLength(route, 129.0);
  ...
  TEST_EQUAL(t[5].m_pedestrianTurn, PedestrianDirection::ReachedYourDestination, ());
}
