...
struct RoundaboutInfo
{
  ...
  // Angular sweep over the contiguous annotated roundabout path, rounded to degrees in [1, 360].
  // 0 means that the geometry is unavailable. For a paired entry and exit this is the sweep from
  // entrance to exit. The direction is independent of regional traffic rules.
  uint16_t m_exitAngle = 0;
  RoundaboutDirection m_direction = RoundaboutDirection::Unknown;
  // True when the maneuver leaves the roundabout. It distinguishes an entry-only maneuver from a
  // paired entry and exit when the exit angle is unavailable.
