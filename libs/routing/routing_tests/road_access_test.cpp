...
UNIT_TEST(RoadAccess_WayBlockedWhenStartButOpenWhenReach)
{
  ...
  // interval (arrival +/- 1h) stays blocked and we are sure (3, 4) is closed
  // (the closing minute is exclusive, so an edge exactly on 13:00 would not
  // count as blocked). Since the alternative way is also blocked, no way
  // should be found.
  expectedWeight = 0.0;
  expectedEdges = {};
