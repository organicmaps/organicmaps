...
UNIT_TEST(RoadAccess_WayBlockedWhenStartButOpenWhenReach)
{
  ...
  // This is deep inside the 10:00-13:00 window, so the whole confidence
  // interval (arrival +/- 1h) stays strictly inside it and (3, 4) is surely
  // closed. The closing minute is exclusive: an interval endpoint landing on
  // 13:00 would already count as open and degrade the answer to Maybe. Since
  // the alternative way is also blocked, no way should be found.
  expectedWeight = 0.0;
  expectedEdges = {};
