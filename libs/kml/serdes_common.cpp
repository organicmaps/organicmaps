...
static std::string PointToString(m2::PointD const & org, char const separator)
{
  ...
  constexpr size_t kPrecision = 6;  // 0.11m precision, more than enough for our use cases.
  return float_to_string(lon, kPrecision) + separator + float_to_string(lat, kPrecision);
}
