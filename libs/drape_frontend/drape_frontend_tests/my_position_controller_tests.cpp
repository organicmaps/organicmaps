...
// @returns the bottom offset of the position anchor in routing mode, in pixels.
...
UNIT_CLASS_TEST(VisualParamsFixture, MyPositionController_RoutingOffsetFollowsVisualScale)
{
  ...
  {
    ScopedVisualScale const guard(3.0);
    TEST_ALMOST_EQUAL_ABS(GetRoutingOffsetY(controller), defaultOffset * 3.0, 1e-9, ());
  }
  TEST_ALMOST_EQUAL_ABS(GetRoutingOffsetY(controller), defaultOffset, 1e-9, ());

  // An offset reported by the UI is in pixels and is not scaled, but the arrow's size on top of it is.
  controller.UpdateRoutingOffsetY(false /* useDefault */, 5 /* offsetY */);
  double const uiOffset = GetRoutingOffsetY(controller);
  {
    ScopedVisualScale const guard(3.0);
    TEST_ALMOST_EQUAL_ABS(GetRoutingOffsetY(controller) - uiOffset, df::Arrow3d::GetMaxBottomSize() * 2.0, 1e-9, ());
  }
}
}  // namespace my_position_controller_tests
