...
UNIT_TEST(SmallMap_Smoke)
{
  // Unsorted init to also cover FinishBuilding().
  SmallMap<uint32_t, int> map = {{5, 50}, {1, 10}, {3, 30}};
  TEST_EQUAL(map.size(), 3, ());
  TEST(!map.Find(0), ());
  TEST(!map.Find(6), ());
  TEST_EQUAL(*map.Find(1), 10, ());
  TEST_EQUAL(map.Get(5), 50, ());

  map.Replace(3, 33);
  TEST_EQUAL(*map.Find(3), 33, ());
  SmallMap<uint32_t, int> const expected = {{1, 10}, {3, 33}, {5, 50}};
  TEST(map == expected, ());

  SmallMapBase<uint32_t, int> baseMap;
  baseMap.Insert(7, 70);
  TEST_EQUAL(*baseMap.Find(7), 70, ());
  TEST(!baseMap.Find(8), ());
}
...
{
  size_t constexpr kCount = 10000000;
...
UNIT_TEST(SmallMap_Benchmark1)
{
  ...
  // Expect t2 < t1 here, but the margin is too small to assert: it flakes on CI.
...
UNIT_TEST(SmallMap_Benchmark2)
{
  ...
  TEST_EQUAL(sum1, sum2, ());
  // std::hash(std::string) is better than std::less(std::string), the margin is ~2x.
  TEST_LESS(t1, t2, ());
  LOG(LINFO, ("unordered_map time =", t1, "SmallMap time =", t2));
}
#endif
