...
# copy_resources() in qt/CMakeLists.txt fills <build>/OMaps.app/Contents/Resources on every
# platform, so the app is picked by the host OS and not by that folder's existence.
if [ "$(uname -s)" = Darwin ]; then
  APP=OMaps.app
  APP_BINARY="$APP/Contents/MacOS/OMaps"
else
  APP=OMaps
  APP_BINARY="$APP"
fi
