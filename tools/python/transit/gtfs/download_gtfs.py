...
https://bit.ly/catalogs-csv  - Mobility Database
...
URL_MOBILITYDB_GTFS_SOURCE = "https://bit.ly/catalogs-csv"
...
def download_gtfs_sources_mobilitydb(path):
    ...
    try:
        ...
        csv_file = open(os.path.join(path, RAW_FILE_MOBILITYDB), 'wb')
        csv_file.write(url_content)
        csv_file.close()
    except requests.exceptions.HTTPError:
        logger.error(f"HTTP error {http_err} downloading zip from {url}. Update URL_MOBILITYDB_GTFS_SOURCE to a working url.")
...
def get_gtfs_urls_mobilitydb(path):
    ...
    raw_sources = pandas.read_csv(RAW_FILE_MOBILITYDB)
    urls = raw_sources['urls.direct_download']
...
def main():
    ...
    if args.mode in ["fullrun", "load_feed_zips"]:
        ...
        if args.source in ["all", "mobilitydb"]:
            load_gtfs_zips_from_urls(args.path, URLS_FILE_MOBILITYDB, args.threads, "dm")
