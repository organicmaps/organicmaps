    ...
    while i < len(lines):
        ...
        if line.startswith("@") or ":" not in line:
            ...
            while i < len(lines):
                ...
                # Comments within a section (shouldn't happen based on format, but handle it)
            ...
            # Sort the section lines alphabetically (case-insensitive)
            section_lines.sort(key=lambda x: x.lower())
...
Examples:
  ...
  python3.12 sort_categories.py

  # Sort in-place (overwrites the original file)
  python3.12 sort_categories.py --in-place

  # Specify custom input and output files
  python3.12 sort_categories.py -i input.txt -o output.txt
        """,
    )
    ...
    parser.add_argument(
        "--in-place",
        action="store_true",
        help="Sort the file in-place (overwrites the input file)",
    )

    args = parser.parse_args()

    input_file = args.input
    output_file = args.output if not args.in_place else input_file
    ...
    # Create backup if sorting in-place
    if args.in_place:
