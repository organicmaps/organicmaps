...
def _same_json_value(lhs, rhs):
    """Compare parsed JSON without Python's bool/int equality coercion."""
    if type(lhs) is not type(rhs):
        return False
    if isinstance(lhs, dict):
        return list(lhs) == list(rhs) and all(
            _same_json_value(lhs[name], rhs[name]) for name in lhs)
    if isinstance(lhs, list):
        return len(lhs) == len(rhs) and all(
            _same_json_value(a, b) for a, b in zip(lhs, rhs))
    return lhs == rhs
...
def format_json(text):
    ...
    exact = json.loads(text, parse_int=Decimal, parse_float=Decimal, **parse_args)
    data = json.loads(text, **parse_args)
    # ensure_ascii=False keeps Cyrillic/CJK literal instead of \uXXXX escapes.
    formatted = json.dumps(data, indent=2, ensure_ascii=False, allow_nan=False) + "\n"
    round_tripped = json.loads(
        formatted, parse_int=Decimal, parse_float=Decimal, **parse_args)
    if not _same_json_value(exact, round_tripped):
        raise ValueError("JSON number cannot be formatted without changing its value")
    return formatted
...
def format_xml(raw):
    ...
    if _is_unsafe(doc):
        raise SkipFile("mixed content / CDATA / xml:space=preserve")
...
def process(rel, fix):
    ...
    try:
        if rel.endswith(".json"):
            ...
            # Inspect the XML declaration before assuming UTF-8, so supported
            # non-UTF-8 documents can be skipped instead of failing to decode.
    ...
    if formatted == original:
        return "ok"
    if fix:
        # Binary output avoids native-Windows newline translation to CRLF.
