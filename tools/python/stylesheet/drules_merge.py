...
def read_drules(drules):
  ...
  for elem in drules.cont:
    ...
    for el in elem.element:
      zoom = el.scale
      if zoom > 0:
        if not zooms[1] or zoom > zooms[1].scale:
          zooms[1] = el
        if not zooms[0] or zoom < zooms[0].scale:
          zooms[0] = el
    if zooms[0]:
      name = str(elem.name)
      if name in result:
...
def create_diff(zooms1, zooms2):
  ...
  return (add_zooms_low, add_zooms_high, add_types)
