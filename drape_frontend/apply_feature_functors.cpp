...
namespace df
{
dp::Color ToDrapeColor(uint32_t src) { return dp::Color(src, static_cast<uint8_t>(255 - (src >> 24))); }
...
void ApplyLineFeatureGeometry::operator()(m2::PointD const & point)
{
  ...
  {
    if (m_simplify && ((m_spline->GetSize() > 1 && point.SquaredLength(m_lastAddedPoint) < m_minSegmentSqrLength) ||
