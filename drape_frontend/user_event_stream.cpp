...
void TouchEvent::SetFirstTouch(const Touch & touch) { m_touches[0] = touch; }

void TouchEvent::SetSecondTouch(Touch const & touch) { m_touches[1] = touch; }
...
uint8_t TouchEvent::GetFirstMaskedPointer() const { return static_cast<uint8_t>(m_pointersMask & 0xFF); }
...
uint8_t TouchEvent::GetSecondMaskedPointer() const { return static_cast<uint8_t>((m_pointersMask & 0xFF00) >> 8); }
...
ScreenBase const & UserEventStream::GetCurrentScreen() const { return m_navigator.Screen(); }

m2::RectD const & UserEventStream::GetVisibleViewport() const { return m_visibleViewport; }
...
bool UserEventStream::OnSetScale(ref_ptr<ScaleEvent> scaleEvent)
{
  ...
  if (scaleEvent->IsAnim())
  {
    ...
    anim->SetOnFinishAction(
      [this](ref_ptr<Animation> animation)
      {
        if (m_listener)
...
m2::AnyRectD UserEventStream::GetCurrentRect() const { return m_navigator.Screen().GlobalRect(); }
...
void UserEventStream::UpdateTouches(std::array<Touch, 2> const & touches) { m_touches = touches; }
...
void UserEventStream::UpdateDoubleTapAndHold(Touch const & touch)
{
  ...
  double const scaleFactor =
    exp(kPowerModifier * (touch.m_location.y - m_startDoubleTapAndHold.y) / GetCurrentScreen().PixelRectIn3d().SizeY());
...
bool UserEventStream::IsInUserAction() const { return m_state == STATE_DRAG || m_state == STATE_SCALE; }

bool UserEventStream::IsWaitingForActionCompletion() const { return m_state != STATE_EMPTY; }
