...
m2::RectD TileInfo::GetGlobalRect() const { return GetTileKey().GetGlobalRect(); }
...
void TileInfo::ReadFeatureIndex(MapDataProvider const & model)
{
  ...
  model.ReadFeaturesID(
    [this, &lastMwm](FeatureID const & id)
    {
...
void TileInfo::Cancel() { m_isCanceled = true; }
...
bool TileInfo::IsCancelled() const { return m_isCanceled; }

bool TileInfo::DoNeedReadIndex() const { return m_featureInfo.empty(); }

int TileInfo::GetZoomLevel() const { return ClipTileZoomByMaxDataZoom(m_context->GetTileKey().m_zoomLevel); }
}  // namespace df
