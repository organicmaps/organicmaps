...
void DrapeEngine::AddTouchEvent(TouchEvent const & event) { AddUserEvent(make_unique_dp<TouchEvent>(event)); }
...
void DrapeEngine::MakeFrameActive() { AddUserEvent(make_unique_dp<ActiveFrameEvent>()); }
...
dp::DrapeID DrapeEngine::GenerateDrapeID() { return ++m_drapeIdGenerator; }
...
void DrapeEngine::AddUserEvent(drape_ptr<UserEvent> && e) { m_frontend->AddUserEvent(std::move(e)); }
...
location::EMyPositionMode DrapeEngine::GetMyPositionMode() const { return m_frontend->GetMyPositionMode(); }
...
void DrapeEngine::SetModelViewListener(ModelViewChangedHandler && fn) { m_modelViewChangedHandler = std::move(fn); }
...
void DrapeEngine::SetTapEventInfoListener(TapEventInfoHandler && fn) { m_tapEventInfoHandler = std::move(fn); }
...
void DrapeEngine::SetFontScaleFactor(double scaleFactor) { VisualParams::Instance().SetFontScale(scaleFactor); }
