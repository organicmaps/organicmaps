...
bool RemoveGroups(ToDo && filter, std::vector<drape_ptr<RenderGroup>> & groups, ref_ptr<dp::OverlayTree> tree)
{
  ...
  auto it = std::remove_if(groups.begin(), itEnd,
                           [&filter, tree](drape_ptr<RenderGroup> & group)
                           {
                             if (filter(group))
                             {
  ...
  , m_renderInjectionHandler(std::move(params.m_renderInjectionHandler))
{
  ...
  m_gpsTrackRenderer = make_unique_dp<GpsTrackRenderer>(
    [this](uint32_t pointsCount)
    {
  ...
  m_routeRenderer = make_unique_dp<RouteRenderer>(
    [this](uint32_t pointsCount)
    {
...
FrontendRenderer::~FrontendRenderer() { ASSERT(m_isTeardowned, ()); }
...
std::unique_ptr<threads::IRoutine> FrontendRenderer::CreateRoutine() { return std::make_unique<Routine>(*this); }
...
void FrontendRenderer::InvalidateRect(m2::RectD const & gRect)
{
  ...
  if (rect.Intersect(screen.ClipRect()))
  {
    ...
    CalcTilesCoverage(rect, dataZoomLevel,
                      [this, &rect, &tiles](int tileX, int tileY)
                      {
...
void FrontendRenderer::RemoveRenderGroupsLater(TRenderGroupRemovePredicate const & predicate)
{
  ...
  {
    RemoveGroups(
      [&predicate, &layer](drape_ptr<RenderGroup> const & group)
      {
        if (predicate(group))
        {
          group->DeleteLater();
...
bool FrontendRenderer::CheckTileGenerations(TileKey const & tileKey)
{
  ...
  RemoveRenderGroupsLater(
    [&tileKey](drape_ptr<RenderGroup> const & group)
    {
...
bool FrontendRenderer::HasTransitRouteData() const { return m_routeRenderer->HasTransitData(); }

bool FrontendRenderer::HasRouteData() const { return m_routeRenderer->HasData(); }
...
void FrontendRenderer::OnDragStarted() { m_myPositionController->DragStarted(); }
...
void FrontendRenderer::OnScaleStarted() { m_myPositionController->ScaleStarted(); }

void FrontendRenderer::OnRotated() { m_myPositionController->Rotated(); }

void FrontendRenderer::OnScrolled(m2::PointD const & distance) { m_myPositionController->Scrolled(distance); }

void FrontendRenderer::CorrectScalePoint(m2::PointD & pt) const { m_myPositionController->CorrectScalePoint(pt); }
...
FrontendRenderer::Routine::Routine(FrontendRenderer & renderer)
  : m_renderer(renderer)
{}
...
void FrontendRenderer::OnEnterBackground() { m_myPositionController->OnEnterBackground(); }
...
void FrontendRenderer::ScheduleOverlayCollecting()
{
  ...
  m_notifier->Notify(ThreadsCommutator::RenderThread, kCollectingEventsPeriod, true /* repeating */,
                     [this](uint64_t)
                     {
