...
void RouteRenderer::PrepareRouteArrows(ScreenBase const & screen, PrepareRouteArrowsCallback const & prepareCallback)
{
  for (auto & subrouteInfo : m_subroutes)
  {
    ...
    dp::DrapeRoutine::RunSequential(
      [subrouteId, screenRect, screenScale, halfWidth, subroute = std::move(subroute), subrouteLength, dist,
       prepareCallback]()
      {
...
RouteRenderer::Subroutes const & RouteRenderer::GetSubroutes() const { return m_subroutes; }
...
void RouteRenderer::UpdateDistanceFromBegin(double distanceFromBegin) { m_distanceFromBegin = distanceFromBegin; }

void RouteRenderer::SetFollowingEnabled(bool enabled) { m_followingEnabled = enabled; }
...
void RouteRenderer::RemovePreviewSegment(dp::DrapeID id) { m_previewSegments.erase(id); }

void RouteRenderer::RemoveAllPreviewSegments() { m_previewSegments.clear(); }
...
bool RouteRenderer::HasData() const { return !m_subroutes.empty(); }

bool RouteRenderer::HasPreviewData() const { return !m_previewSegments.empty() && !m_previewRenderData.empty(); }
}  // namespace df
