                                                                         ...
                                                                         MapDataProvider & mapDataProvider)
{
  ...
  if (spline.IsNull())
  {
    mapDataProvider.ReadFeatures(
      [&points](FeatureType & ft)
      {
  ...
  for (size_t i = 0; i < segsCount; ++i)
  {
    ...
    {
      std::vector<glsl::vec2> normals =
        GenerateCapNormals(dp::RoundCap, segments[i].m_leftNormals[StartPoint], segments[i].m_rightNormals[StartPoint],
