...
/// @{
template <class Sink> void WriteVarUInt32SortedShortArray(std::vector<uint32_t> const & cont, Sink & sink)
{
  CHECK(!cont.empty(), ());
...
template <class Source, class ContT> void ReadVarUInt32SortedShortArray(Source & src, ContT & cont)
{
  cont.clear();
